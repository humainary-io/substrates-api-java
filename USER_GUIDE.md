...
Substrates defines interfaces organized into functional categories:

#### Data Flow Interfaces

- **Pipe**: Primary emission carrier with `emit(E)` method and `subject()` for identity
- **Channel**: Subject-based port into conduit pipeline, creates pipes (@Temporal - only valid in
  Composer callback)
- **Flow**: Type-preserving transformation operators (diff, guard, limit, sample, sift, reduce,
  etc.)
- **Sift**: Comparison-based filtering within Flow pipelines (@Temporal - don't retain)

#### Component Interfaces

- **Circuit**: Central processing engine, single-threaded event loop
- **Conduit**: Caches percepts by name (via Lookup), applies flow operators
- **Cell**: Hierarchical computational unit for stack-safe deep hierarchies
- **Cortex**: Factory for creating circuits, scopes, slots, and states

#### Identity Interfaces

- **Subject**: Hierarchical reference with identity, name, state (parameterized by substrate type)
- **Name**: Hierarchical identifier (e.g., "service.api.latency"), interned for identity equality
- **Id**: Unique identity token with reference equality
- **Substrate**: Base abstraction for all typed entities with associated subjects
- **Extent**: Hierarchical nesting abstraction (implemented by Name, Subject, Cell, Scope)

#### Resource Interfaces

- **Resource**: Lifecycle interface with idempotent `close()` method
- **Scope**: Hierarchical resource container with automatic cleanup (implements AutoCloseable)
- **Closure**: Block-scoped resource management via `consume(Consumer)` (@Temporal)
- **Reservoir**: In-memory buffer capturing emissions as Captures

#### Discovery Interfaces

- **Source**: Event-emitting components with subscription capability
- **Lookup**: Read-only percept lookup by name (implemented by Conduit, Cell)
- **Subscriber**: Dynamically subscribes to channels and registers pipes
- **Subscription**: Handle for canceling subscription via `close()`
- **Registrar**: Callback interface for registering pipes (@Temporal)

#### Configuration Interfaces

- **Composer**: Functional interface composing percepts from channels
- **Configurer**: Callback for configuring temporal objects (Flow, Sift)

#### Utility Interfaces

- **State**: Immutable collection of named slots with persistent updates
- **Slot**: Type-safe name-value pair with fallback value
...
- **Capture**: Emission record with subject and value (produced by Reservoir)
...
// (This example shows what NOT to do in a receptor callback)
circuit.pipe(value -> {
...
```
// Use transit queue priority instead - emissions from circuit thread
// go to transit queue which processes before ingress queue
final Pipe<Integer> asyncPipe = circuit.pipe(value -> {
  // Emit recursively - this goes to transit queue
  otherPipe.emit(derivedValue);
  // Transit queue ensures it processes before next ingress item
});
```
...
### Taps: Transformed Views

A **Tap** creates a transformed view of a conduit's emissions. It mirrors the channel structure
but transforms values from type E to type T:

```
// Create tap that transforms integers to strings
final Tap<String> tap = conduit.tap(
  i -> "value:" + i
);

// Subscribe to receive transformed emissions
tap.subscribe(subscriber);
...
Taps can also apply flow operations to the transformed emissions:
...
- **Channel mirroring**: Tap channels correspond 1:1 with source conduit channels
- **Type transformation**: Mapper converts emissions from E to T
- **Flow support**: Optional configurer applies filtering/stateful operations
- **Resource lifecycle**: Tap extends `Resource` and must be closed when no longer needed
- **Thread safety**: Mapper and flow execute on circuit thread (no synchronization needed)
...
Subscribers are notified when channels are **created or rebuilt**. Subscribers are created from a
**Circuit** via `circuit.subscriber()`, making them children of that circuit in the subject
hierarchy. This establishes that the subscriber observes the ordered view provided by the circuit's
single-threaded processing:

```
// Create a subscriber scoped to this circuit
// Subject hierarchy: circuit-name.my.subscriber
final Subscriber<Integer> subscriber = circuit.subscriber(
  cortex.name("my.subscriber"),
  (subject, registrar) -> {
    System.out.println("Channel discovered: " + subject.name());

    // Register a receptor to receive emissions
    registrar.register(value -> System.out.println("Received: " + value));
  }
);
...
subscriber = circuit.subscriber(name, (subject, registrar) -> {
...
```
capture.subject();   // Subject<Channel<E>> - which channel emitted
...
flow.sift(Comparator.naturalOrder(),     // Comparison-based filtering
  sift -> sift.above(0).below(100))      // Range: 0 < value < 100
...
Pipe<Integer> tail = circuit.pipe(value ->
...
pipes[0] = circuit.pipe(value -> {
  if (counter.incrementAndGet() < 10) {
    pipes[1].emit(value + 1);  // A → B
  }
});

pipes[1] = circuit.pipe(value -> {
  pipes[2].emit(value + 1);  // B → C
});

pipes[2] = circuit.pipe(value -> {
...
**Cell** provides stack-safe hierarchical computation with async upward flow:

```
// Create a cell with ingress (input→output transform) and egress (output aggregation)
final Cell<Integer, Integer> cell = circuit.cell(
  cortex.name("nested.cell"),
  (subject, outlet) -> outlet,           // ingress: pass through
  (subject, outlet) -> outlet,           // egress: pass through
  value -> System.out.println("Got: " + value)  // receptor for aggregated output
);

// Emit to cell input
cell.emit(42);

// Get child cells by name (creates if needed)
Cell<Integer, Integer> child = cell.percept(cortex.name("child"));
```

Cells enable:

- **Stack-safe deep hierarchies** (100+ levels via async dispatch)
- **Hierarchical decomposition** of complex systems
- **Upward aggregation** from children to parents
...
**10. Situations** - Situational assessment (NORMAL, WARNING, CRITICAL)
...
// Create observer (scoped to circuit)
final var observer = circuit.subscriber(
...
// Observe conditions
final var monitorReservoir = monitorConduit.reservoir();
...
```
// Create a pipe with error handling
final Pipe<Value> safePipe = circuit.pipe(value -> {
  try {
    processValue(value);
  } catch (Exception e) {
...
```
// Calling await() inside a circuit callback
final Pipe<Integer> badPipe = circuit.pipe(value -> {
  circuit.await();  // ❌ IllegalStateException!
});
```
...
**Solution**: Don't call `await()` from within circuit callbacks. Use transit queue priority for
recursive emissions.
...
// Emit values (subscriber no longer receives after lazy rebuild)
pipe.emit(value);
```
...
// Create a pipe on c1 that forwards to c2
final Pipe<Integer> bridge = c1.pipe(value -> {
  pipe2.emit(value * 2);  // c1 → c2 cross-circuit emission
});
...
// Query state using Slot
Slot<Integer> alphaSlot = cortex.slot(cortex.name("alpha"), 0);
int value = state.value(alphaSlot);  // 1 (or 0 if not found)
...
```
// Use guard for simple predicates
final Conduit<Pipe<Integer>> positiveConduit = circuit.conduit(
  cortex.name("positive"),
  Composer.pipe(Integer.class),
  flow -> flow.guard(value -> value > 0)
);

// Use sift for comparison-based filtering
final Conduit<Pipe<Integer>> rangeConduit = circuit.conduit(
  cortex.name("ranged"),
  Composer.pipe(Integer.class),
  flow -> flow.sift(
    Comparator.naturalOrder(),
    sift -> sift.above(10).below(100)  // 10 < value < 100
  )
);

// Use peek for observing emissions without modifying flow
final Conduit<Pipe<Integer>> observingConduit = circuit.conduit(
  cortex.name("observing"),
  Composer.pipe(Integer.class),
  flow -> flow
    .guard(value -> value > 100)
    .peek(highPriorityHandler::accept)  // Observe high-priority values
);
```
...
// Layer 3: Observer aggregation (scoped to circuit)
final var observer = circuit.subscriber(
...
// Layer 4: Automated actions (scoped to circuit)
final var actionObserver = circuit.subscriber(
  cortex.name("actions"),
  (subject, registrar) -> {
    registrar.register(signal -> {
