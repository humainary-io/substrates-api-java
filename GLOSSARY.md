...
* **Cortex**: The factory entry point for the Substrates framework. Provides methods for creating
  circuits (`circuit()`), hierarchical names (`name()`), scopes (`scope()`), slots (`slot()`), and
  states (`state()`). Access via `Substrates.cortex()`.
...
* **Pipe**: An emission carrier responsible for passing typed values through pipelines. Extends the
  ...
  `Circuit.pipe(Receptor)` to wrap callbacks for receiving emissions. Provides `emit(E)` for sending
  values.
...
* **Fiber**: A per-emission operator pipeline preserving emission type. Created via
  ...
  control (`every`, `chance`, `skip`, `limit`, `inhibit`, `delay`), stateful operators (`diff`,
  `distinct`, `distinct(int)`, `reduce`, `integrate`, `rolling`, `pulse`, `relate`, `steady`,
  `tumble`), and observation (`peek`, `replace`, `tee`). Composes with another fiber via
  `fiber(Fiber<E>)` and materializes to a target via `pipe(Pipe<E>)`.
...
* **Flow**: A left-to-right composition surface for type-changing stages and output-side recipes.
  ...
  emission), `scan(Supplier, BiFunction, Function)` for stateful type-changing folds,
  `window(int)` for bounded rolling temporal windows over surviving output values,
...
* **Window**: A callback-scoped, read-only rolling view emitted by `Flow.window(int)`. Exposes the
  most recent surviving output values in encounter order and supports `first`, `last`, `slice`,
  `skip`, `trim`, `reverse`, `size`, iteration, and `stream()`. A Window must be consumed during the
  callback that observes it; copy values if they must be retained or forwarded.
...
* **Registrar**: A temporary handle passed to subscribers during their callback, allowing them to
  ...
  **: Only valid during the subscriber callback — attempting to use it afterwards violates the
  temporal contract.
...
* **Pool**: A composable name-based view extending `Lookup`. Inherits `get(Name)` and adds
  ...
  views that apply a transformation function per name. `Conduit<E>` extends
  `Pool<Pipe<E>>`.
...
* **Bank**: A closeable name-indexed holder for same-kind named resources. Created via
  ...
  conduits it has materialized. Unlike a Pool (a derived view over an existing conduit's pipes), a
  Bank creates and owns its conduits. `get` is open-required — calling it after the bank has been
  closed throws `Fault`.
...
* **Resource**: A lifecycle interface for explicitly releasing resources and terminating operations.
  ...
  from the circuit thread). Implemented by Circuit, Conduit, Bank, Subscription, Reservoir, and
...
* **Capture**: A record of an emitted value from a named pipe with its associated subject. Produced
  by Reservoirs. Provides `emission()` for the value and `subject()` for the pipe identity. Used for
...
* **Purpose**: append a downstream type transformation, run a stateful type-changing fold, attach
  Fiber recipes at the output side, emit rolling windows, or compose flows
* **Operations**: `map`, `scan`, `window`, `fiber(Fiber)`, `fiber(Function<Subject,Fiber>)`,
