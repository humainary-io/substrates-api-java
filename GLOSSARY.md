...
* **Cell**: A circuit-owned, initialized, single-slot state holder with safe publication. Created
  via `Circuit.cell(E)` with a non-null seed value (the no-argument factory was removed in 2.9).
...
* **Port** (2.9): A circuit-owned, initialized state slot that grants queued mutation authority
  ...
  `emit(Pipe<? super E>)` — all queued, all callable from any context. Holders can evolve state but
...
* **Pin** (2.9): A circuit-owned, initialized state handle that grants immediate inspect/mutate
...
* **Basin**: A circuit-owned, bounded in-memory buffer of values fed through `Basin.pipe()`; the
  ...
  `capacity` values and provides `drain(Pipe)` to forward them to a target pipe (consuming the
  buffer) on the circuit worker thread. Holds plain values; to capture a source's emissions with
  their subjects, feed a `Basin<Capture<E>>` from a `Sink`.
...
* **Resource**: A lifecycle interface for explicitly releasing resources and terminating operations.
  ...
  from the circuit thread). Implemented by Circuit, Conduit, Bank, Subscription, and Subscriber.
...
* **Operations**: `map`, `scan`, `window(int)`, `window(Duration, int)`, `fiber(Fiber)`,
  `fiber(Function<Subject,Fiber>)`,
