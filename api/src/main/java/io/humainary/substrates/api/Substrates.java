...
public interface Substrates {
  ...
  interface Capture < E > {
    ...
    /// subscriber), the sink's channel mirrors the source channel's name but keeps its own identity, as
    /// with [Tap] mirror pipes.
    ///
  ...
  interface Circuit
    ...
    ///   [#basin(int)], [#tap(Function)] and friends — raise [Fault] on
    ...
    /// @throws Fault                if any target is not a runtime-provided implementation
    /// @see #pipe(Pipe)
    /// @since 3.0
    ...
    /// @throws Fault                if name or any target is not a runtime-provided implementation
    /// @see #pipe(List)
    /// @since 3.0
    ...
    /// @throws NullPointerException if any argument is `null`
    /// @throws Fault                if name is not from this runtime provider
    /// @see #pipe(Receptor)
    /// @since 2.7
  ...
  ///   `Conduit.tap(...)`, `Circuit.basin(int)`) raise [Fault] on the caller
