...
public interface Substrates {
  ...
  non-sealed interface Channel < E >
    ...
    ///
    /// This method creates a pipe with a custom flow configuration. On the circuit
    /// thread, the per-pipe flow is applied first, followed by the conduit-level flow,
    /// before emissions reach the channel. The configurer allows flow transformations
  ...
  non-sealed interface Circuit
    ...
    default < P extends Percept, E > Conduit < P, E > conduit (
      @NotNull final Composer < E, ? extends P > composer,
      @NotNull final Configurer < Flow < E > > configurer
    ) {
    ...
    /// @param target the pipe that will receive emissions asynchronously
    /// @param <E>    the class type of emitted values
    /// @return A pipe with the specified name that asynchronously dispatches to the target pipe
    ...
    < E > Subscriber < E > subscriber (
      @NotNull Name name,
      @NotNull BiConsumer < Subject < Channel < E > >, Registrar < E > > subscriber
    );
  ...
  non-sealed interface Conduit < P extends Percept, E >
    ...
    /// @param mapper the function to transform emissions from E to T
  ...
  interface Flow < E > {
    ...
    Flow < E > sift (
      ...
      @NotNull Configurer < Sift < E > > configurer
    );
