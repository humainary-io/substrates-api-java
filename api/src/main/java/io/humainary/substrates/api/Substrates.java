...
public interface Substrates {
  ...
  ///
  /// Captures are produced by two substrates: a [Reservoir], which observes a source and records each
  /// emission into a bounded buffer drained on demand (see [Reservoir#drain()]); and a [Sink], a
  /// capture-producing pool of channels — the output-closed dual of a [Conduit] — that forwards each
  /// capture to a fixed endpoint. The two differ in what [#subject()] reports — see that accessor.
  ///
  /// ## Primary Use Cases
  ///
  ...
  ///
  /// Reservoirs buffer captures in memory up to an explicit capacity. For long-running scenarios:
  ...
  interface Capture < E > {
    ...
    ///
    /// Which pipe this is follows from the producer. A [Sink] is a pool of channels used like a
    /// [Conduit], so a sink capture is attributed to the **sink channel the value was emitted into** —
    /// the channel owns its emissions, exactly as a conduit pipe does. A [Reservoir], by contrast, is a
    /// recorder that observes a source, so a reservoir capture reports the **originating pipe of the
    /// observed source**. The two coincide for a value emitted directly into a sink channel; they
    /// diverge when a sink is bridged onto a source (via the pool-backed subscriber), where the sink's
    /// channel mirrors the source channel's name but keeps its own identity, as with [Tap] mirror pipes.
    ///
  ...
  interface Circuit
    ...
    ///   [#reservoir(int)], [#tap(Function)] and friends — raise [Fault] on
    ...
    /// Returns a named pipe that asynchronously dispatches emissions to a receptor
    /// via this circuit's event queue.
    ///
    ...
    /// @param receptor the receptor that will receive emissions asynchronously
    /// @param <E>      the class type of emitted values
    /// @return A named pipe that asynchronously dispatches to the receptor
    /// @throws NullPointerException if any argument is `null`
  ...
  /// A bounded in-memory buffer of captures that is also a Substrate.
  ///
  ...
  /// source to capture emissions into an internal buffer capped by the capacity
  /// supplied at creation time. When the reservoir is full, newer captures evict
  /// the oldest buffered captures.
  ///
  /// @param <E> the class type of the emitted value
  /// @see Capture
  /// @see Source#reservoir(int)
  /// @see Resource
  ...
  interface Reservoir < E >
    ...
    /// reservoir was created or the last call to this method, subject to this
    /// reservoir's capacity.
    ///
    /// **Thread Safety**: This method must be called from a single thread only.
    /// Emissions from the circuit thread may occur concurrently. Callers can drain
    /// repeatedly to retrieve retained items; if more than the reservoir capacity
    /// accumulates between drains, older captures are evicted.
    ///
    /// A drain requested from outside the circuit is ordered behind already
    /// accepted emissions on that circuit. Callers can therefore emit values and
    /// call `drain()` directly; a separate [Circuit#await()] is not required just
    /// to make those prior emissions visible to the drain result.
    ///
  ...
  /// emissions — rather than recorded by observing a source the way a [Reservoir] does. Because only
  /// the runtime can observe the per-emission emitting context, the minted capture carries
  ...
  interface Source < E, S extends Source < E, S > >
    ...
    ///
    /// A reservoir is a bounded in-memory buffer that captures emissions from this
    /// source's channels. Use [Reservoir#drain()] to retrieve accumulated emissions
    /// as a stream of [Capture] objects containing both the emission value and its
    /// source channel subject.
    ///
    /// ## Usage Pattern
    ///
    /// ```java
    /// // Create reservoir for a conduit
    /// Reservoir<Integer> reservoir = conduit.reservoir(32);
    ///
    /// // Emit some values
    /// pipe.emit(1);
    /// pipe.emit(2);
    ...
    ///
    /// The `drain()` method is **incremental** - each call returns only retained
    /// emissions captured since the last drain, not all emissions ever captured.
    /// If more than `capacity` emissions accumulate before a drain, the oldest
    /// captures are evicted and only the newest `capacity` captures are returned.
    ///
    /// ## Drain Synchronization
    ///
    /// Calling [Reservoir#drain()] from outside the circuit is itself ordered with
    /// the circuit queue. Emissions accepted before the drain request are processed
    /// before the drain snapshot is returned, so callers do not need to call
    /// [Circuit#await()] immediately before `drain()` just to observe those emissions.
    ///
    /// ## Resource Lifecycle
    ///
    ...
    ///
    /// @param capacity the maximum number of recent captures retained between drains
