...
public interface Substrates {
  ...
  interface Circuit
    ...
    /// - Synchronous `@New` factory operations that produce a [Resource] —
    ///   [#conduit(Name, Class)], [#subscriber(Name, BiConsumer)],
  ...
  interface Flow < I, O > {
    ...
    /// ## Materialization
    ///
    /// The factory is called exactly once per [#pipe(Pipe)] attachment. Each
    /// attachment receives an independent fiber recipe and its own per-emission
    /// state. The factory MUST produce a fresh recipe on each call: sharing
    /// mutable state across factory invocations is forbidden, the same way that
    /// captured mutable state is forbidden in [#map(Function)] mappers, because
    /// the captured state would be aliased across all materializations.
    ///
    /// ## Exception Handling
    ///
    ...
    /// @throws Fault                if the factory returns a fiber that is not a runtime-provided implementation when the returned flow is materialized
    /// @see Fiber
    /// @see Subject
    /// @see #fiber(Fiber)
    /// @since 2.4
  ...
  interface Pool < T > {
    ...
    /// for subsequent lookups of the same name. This enables composable
    /// transformations:
    ///
    /// ```java
    /// // Domain wrapping:
    ...
    /// (the [#get(Name)] return is `@NotNull`).
    ///
  ...
  /// failure is reported according to the circuit's callback-isolation rules, registrations
  /// completed before the throw remain registered, and the callback is not retried for that
  /// subscription/channel pair.
  ///
