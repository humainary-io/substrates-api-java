...
document explains the design in accessible terms for developers evaluating, using, or reviewing
the API.
...
| Flow    | Transformation pipeline (guard, diff, limit, every, chance, reduce) |
...
| Configurer | Callback interface for temporal configuration |
| Percept    | Marker for all observable entities            |
...
strict enqueue order. Earlier emissions complete before later ones begin. All observers see the same
sequence. This eliminates an entire class of concurrency bugs — not by managing them with locks and
barriers, but by making them structurally impossible.
...
races
structurally: if only one operation can execute at a time within a circuit, concurrent mutation is
impossible. State accessed within the circuit context requires no synchronization. Developers write
sequential logic; the framework guarantees sequential execution.
...
1. **Non-deterministic mailbox ordering**: When multiple actors send messages to the same target
   ...
   on
   the ingress queue — each run sees a single, consistent sequence.
...
- **Transit queue**: Local queue for emissions originating from the circuit thread during processing

When processing an emission triggers additional emissions from within the circuit thread, those
cascading emissions enter the transit queue, which takes strict priority over the ingress queue.
...
overflow. The transit queue converts recursion to iteration — cascading emissions are enqueued, not
immediately invoked. Processing remains iterative regardless of cascade depth. This is essential
for neural-like computational networks where cyclic signal propagation is the norm.

### Problem 3: Coordination Without Locks

The transit queue is accessed only by the circuit thread — no synchronization required. The ingress
queue requires synchronization only at the enqueue boundary. Once work enters the circuit context,
all processing including cascading is lock-free.

### Relationship to Berkeley Reactor

Both Berkeley Reactor (Lohstroh & Lee, 2019) and Substrates seek to tame actor-model
non-determinism through controlled event scheduling. The key difference is structural: Reactors
focus on scheduling via a global logical clock. Substrates introduces physical routing boundaries
(Cortex, Circuit, Conduit, Pipe) that make topology itself an architectural artifact. The routing
path of a signal is as observable and constrained as its timing.
...
- **Flow.limit()**: Caps the number of emissions processed by a pipeline stage. After the limit,
  the stage is permanently closed.
...
Combined with the rest of the Flow operator vocabulary (high/low watermarks via `high`/`low`,
range filtering via `range`/`clamp`/`above`/`below`/`min`/`max`/`deadband`, change detection via
`diff`/`change`, accumulation via `reduce`/`integrate`/`relate`, transition detection via
`edge`/`pulse`, temporal confirmation via `steady`, gating via `hysteresis`/`inhibit`, temporal
displacement via `delay`, windowed aggregation via `rolling`/`tumble`, and lifecycle windowing via
`dropWhile`/`takeWhile`/`skip`), they compose into complex event processing patterns without
requiring external stream processing engines.
...
`Registrar` carries a temporal contract — it is valid only during the subscriber callback. This
allows implementations to reuse the registrar across callbacks rather than allocating a new
instance per invocation. `Flow`, by contrast, is an immutable builder value: a single Flow
configuration can be shared across many materializations and across threads, so the configuration
itself is allocated once and never copied. In both cases the goal is the same — keep allocation
off the hot path. In a system processing hundreds of millions of emissions per second, eliminating
even one allocation per emission is significant.
...
| Current   | Execution context (thread) | Binds to thread identity; invalid from other threads |
...
2. **Lifecycle clarity**: The temporal contract makes ownership explicit. A Registrar belongs to the
   subscriber callback. Retaining it creates a dangling reference to internal framework state.
...
the subscriber callback is caught and reported. `Current` violations are not actively detected;
misuse produces undefined behavior, since enforcement would require per-access checks on the hot
path.
...
rebuilds — most for channels that may never emit again. This creates a latency spike proportional
...
- **Hierarchical resource management**: Closing a circuit closes its conduits and their channels.
  Teardown mirrors construction order.
- **Scoped determinism**: Each circuit is an independent deterministic domain. Cross-circuit
  communication goes through emission (enqueue to the target's ingress queue), preserving
