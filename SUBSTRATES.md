...
subscription-based wiring, and single-digit-nanosecond emission latency.

Every type in the API exists for a specific structural reason — performance,
...
The types are not equally central. Most users need only the core layer to build with
...
| Routing | Conduit dispatch mode — `PIPE` (target only), `STEM` (leaf-first ancestors)   |
| Sink    | Named pipe pool that funnels captures to a fixed endpoint                     |
| Pipe    | Emission carrier — `emit(value)`                                              |
| Cell    | Circuit-owned single-slot state holder — `cell.pipe()` updates, `cell.get()`  |
| Port    | Circuit-owned state slot — queued mutation authority, no read accessor        |
| Pin     | Circuit-confined state handle — immediate `get()`/`set()` on the owner thread |
| Ticker  | Circuit-owned periodic emitter — `circuit.ticker(Duration, Pipe)`             |
...
| Run     | Value plus its consecutive-run length, emitted by Flow.run                    |
| Change  | Run-boundary envelope (from, to, closed length) from Flow.change              |
...
| Basin    | Bounded value buffer (Cell's multi-value sibling) |
| Capture  | Emission + subject pair (produced by a Sink)      |
| Pulse    | Circuit round-trip timing snapshot (diagnostics)  |
...
| Source       | Subscription management interface (Conduit)                     |
| Lookup       | Composable name-based retrieval with caching                    |
| Pool         | Composable derived views over a name→pipe pool                  |
...
| Bank     | Closeable name-indexed conduit holder   |
...
A Sink feeding a Basin is the concrete mechanism: a sink subscribed to a source mints a Capture for
every emission — alongside the channel subject that produced it, preserving circuit-thread
processing order — and a `Basin<Capture<E>>` buffers them. Its drain forwards the buffered captures
to a target pipe and clears the buffer, returning only the captures accumulated since the last
drain.
...
- **Fiber.every () / Fiber.chance ()**: Filter emissions by interval (every Nth count or every
  elapsed processing-time `Duration`) or probability respectively. Reduce downstream volume without
  affecting ingress.
...
The API is composed of many small interfaces. This section explains why.

### The Capability Hierarchy

Three interfaces layer one capability each, each level a subset of the one above:

```text
Substrate  (identity  — has subject())
  └── Resource  (lifecycle — adds close())
        └── Source  (wiring — adds subscribe())
```

Only `Substrate` is sealed (the identity root); `Resource` and `Source` are open so the SPI and
projections can supply concrete component types. Not every substrate owns resources (a `Pipe` is
GC-collected, never closed), and not every resource manages subscriptions (a `Bank` has lifecycle
but no subscription model). The component types at each level — the specification's capability
matrix is the authoritative inventory:

- **Substrate** (`subject()`): Cortex, Circuit, Conduit, Sink, Bank, Cell, Port, Pin, Ticker, Pipe,
  Current, Scope, Basin, Subscriber, Subscription
- **Resource** (adds `close()`): Circuit, Conduit, Bank, Ticker, Subscriber, Subscription
- **Source** (adds `subscribe()`): Conduit
...
Interface segregation makes this precise: each capability is its own interface, so a type extends
only the capabilities it provides. Collapsing the hierarchy would force every type to carry
...
full identity, hierarchy, and state information through the dispatch path. At the emission rates
this path is built for, every unnecessary field access matters.
...
- **Circuit** extends Resource: owns the processing thread and the lifecycle of circuit-owned
  components
...
The design targets emission latency in the single-digit-to-low-double-digit nanosecond range, and
that target drives optimization decisions throughout. For the figure a given provider actually
achieves on a given machine, run the JMH benchmarks rather than reading a number from prose — a
number frozen in a document becomes a target that outlives the machine that produced it.
...
- **Hierarchical resource management**: Closing a circuit closes the components it owns, including
  its conduits. Teardown mirrors construction order. Pipes are not among them — `Pipe` does not
  extend Resource; pipe references stay valid after their conduit closes and are reclaimed by
  garbage collection, not by an explicit close.
...
topology via subscriptions, nanosecond-scale emission latency — were shaped by these aspirations.
