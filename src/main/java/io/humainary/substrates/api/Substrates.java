/// Copyright © 2025–2026 William David Louth
...
///
/// - Every circuit owns exactly **one sequential execution context**
...
///
/// - **[Cell]**: Circuit-owned state cell updated only through pipes and safely
...
///     chance, change, clamp, deadband, delay, diff, distinct, dropWhile, edge, every,
///     guard, heartbeat, high, hysteresis, inhibit, integrate, limit, low, max, min, peek,
///     pulse, range, reduce, relate, replace, rolling, route, skip, steady, streak,
///     takeWhile, tee, tumble, when.
///
...
///     type-changing fold), `relate` (appends a fold over consecutive pairs), `window`
///     (emits bounded rolling windows), `run` (turns each admission into a [Run] carrying its
///     run length), `change` (emits a [Change] at each run boundary), `fiber` (attaches a
///     same-type Fiber at the output side), and `flow` (composes with another Flow,
///     optionally via a subject-aware factory).
///
...
///
/// The API is composed of many small interfaces. This is deliberate interface segregation, not fragmentation.
...
/// identity checks. Keeping them separate is what holds the emission path down to the latency
/// the runtime targets.
///
...
///
/// **Not Standard Patterns.** The emission-latency target requires that
...
public interface Substrates {
  ...
  @Provided
  @SpecRef ( {"10.4", "16.2"} )
  non-sealed interface Bank < R extends Resource < R > >
    extends Lookup < R >,
            Resource < Bank < R > > {

    /// Returns the resource for the given name, materializing it on first access.
    ///
    /// ## Rejection and attribution
    ///
    /// Two different receivers can reject this call, and [Fault#subject()] identifies
    /// whichever one actually did:
    ///
    /// - **This bank has been closed**: the bank's own open-guard rejects, and the
    ///   fault carries this bank's [Subject].
    /// - **This bank is open but its owning [Circuit] has been closed**: a bank is
    ///   not closed by its circuit closing, so the bank's own guard passes. A name
    ///   this bank has already materialized is returned from the cache and no fault
    ///   is raised. A name it must still materialize reaches the circuit's conduit
    ///   factory ([Circuit#conduit(Name, Class, Routing)]), whose open-guard rejects
    ///   and carries the *circuit's* [Subject] rather than this bank's. That fault
    ///   propagates out of this call unchanged; it is not re-attributed to the bank.
    ///
    ...
    ///                              if this bank has been closed; or if materializing a new
    ///                              resource requires an owning circuit that has been closed
  ...
  /// A bounded, circuit-owned in-memory buffer of values fed through a pipe.
  ///
  /// A Basin is the multi-value sibling of [Cell]: where a cell holds the single
  /// latest value, a basin retains the most recent `capacity` values emitted
  /// through its [#pipe()], in emission order. It is created from a [Circuit]
  /// (see [Circuit#basin(int)]) and holds whatever type `E` is emitted — it does
  /// **not** itself create [Capture]s. To buffer captures, place a [Sink] in front
  /// of the basin (`circuit.sink(basin.pipe())`) so the sink mints the capture and
  /// the basin (a `Basin<Capture<E>>`) holds it.
  ///
  /// ## Bounded retention
  ///
  /// The basin retains at most `capacity` values. When a value is accepted while
  /// the buffer is full, the oldest retained value is evicted before the new one
  /// is recorded.
  ///
  /// ## Reading
  ///
  /// The buffer is read asynchronously on the owning circuit's worker thread,
  /// consuming the retained values there — no buffered state is exposed across the
  /// thread boundary. [#drain(Pipe)] forwards each retained value to a target pipe,
  /// taking the same-circuit transit fast path when the target shares this basin's
  /// circuit and a cross-circuit emission otherwise. There is no synchronous
  /// accessor; to consume with arbitrary logic, drain into a receptor adapted as a
  /// pipe via [Circuit#pipe(Receptor)].
  ///
  /// @param <E> the class type of the buffered value
  /// @see Circuit#basin(int)
  /// @see Cell
  /// @see Sink
  ...
  @Provided
  @SpecRef ( {"11.1", "16.2"} )
  non-sealed interface Basin < E >
    extends Substrate < Basin < E > > {

    /// Drains the retained values into the given pipe on the owning circuit's
    /// worker thread.
    ///
    /// This call enqueues a drain on the circuit and returns immediately; each
    /// retained value is forwarded to `pipe` in emission order, after which the
    /// drained values are evicted. The target need not belong to this basin's
    /// circuit: when it does, forwarding is worker-local transit work; when it
    /// does not, each value is delivered as a cross-circuit emission.
    ///
    /// Draining is itself a queued emission, not a lifecycle operation: a drain
    /// enqueued after the owning circuit has accepted close is silently dropped
    /// (like any emission during shutdown), not guaranteed to run.
    ///
    /// @param pipe the pipe each retained value is forwarded to
    /// @throws NullPointerException if the pipe is `null`
    /// @throws Fault                if the pipe is not a runtime-provided implementation
    /// @see Circuit#pipe(Receptor)
    ...
    /// Returns the pipe that feeds this basin.
    ///
    /// Each value emitted through the returned pipe is queued on the owning
    /// circuit and appended to the basin's bounded buffer on that circuit's worker
    /// thread, evicting the oldest retained value when at capacity. Repeated calls
    /// return the same pipe instance.
    ///
    /// @return a pipe that appends emissions to this basin
  ...
  /// An immutable envelope recording an emitted value together with its provenance.
  ///
  /// Each capture bundles, for one emission:
  /// - the emitted value ([#emission()]) of type E
  /// - the [#subject()] of the pipe (its *channel* role — the inlet side of a named pipe)
  ///   that produced it
  /// - the [#current()] emitting context (the caller for an ingress emission, or the circuit
  ///   itself for a transit cascade)
  /// - a [#state()] of implementation-defined measures (empty unless the provider publishes any)
  ///
  /// The context is exposed as a [Subject], not a live [Current], so a capture is a pure
  /// out-of-execution-scope reference record — safe to retain, compare, and stream without
  /// a handle to any live counterpart. This supports inspecting both what was emitted and
  /// where it came from, which is essential for testing, debugging, and telemetry.
  ///
  /// Captures are produced by a [Sink]: a capture-producing pool of channels — the output-closed dual
  /// of a [Conduit] — that wraps each emission and forwards the capture to a fixed endpoint, for
  /// example a [Basin] that buffers them. What [#subject()] reports follows from the sink channel —
  /// see that accessor.
  ///
  /// ## Primary Use Cases
  ///
  ...
  ///
  /// A [Basin] buffers values in memory up to an explicit capacity. For long-running scenarios:
  /// - Choose a capacity that fits the diagnostic or testing budget
  /// - Call [Basin#drain(Pipe)] periodically to forward buffered values
  /// - Filter or transform within the drain receptor to selectively process captures
  ...
  interface Capture < E > {
    ...
    /// Returns the subject of the execution context that emitted this value.
    ///
    /// This is the [Subject] of the emitting [Current]: the caller context for a value admitted from
    /// outside the circuit (an external thread, or another circuit's worker for a cross-circuit
    /// emission), and the circuit's own context for a value emitted by a transit cascade from within
    /// circuit processing — so for a cascade this coincides with [Circuit#current()]. It is the
    /// immediate emitter, not the ultimate root of a multi-hop chain. Circuit-internal mechanisms
    /// are attributed to the owning circuit rather than to their private machinery — e.g. a [Ticker]
    /// emission reports the owner circuit's context, not the ticker's scheduling thread.
    ///
    /// A capture holds the subject (an out-of-execution-scope reference), not the live [Current], so
    /// consumers inspect identity, name, and state without a handle to the live context. Distinct
    /// from [#subject()]: that is the *pipe* the value was emitted through (the channel), whereas
    /// this is the execution context that performed the emission (the actor).
    ///
    /// @return The subject of the emitting execution context for this emission
    /// @see Current
    /// @see Subject
    /// @see Circuit#current()
    /// @since 2.10
    ...
    /// Returns implementation-defined measures associated with this emission.
    ///
    /// A capture carries a [State] of per-emission measures that the provider may publish —
    /// for example a processing-time reading, queue depth, or other diagnostics — keyed by
    /// [Name]. What, if anything, is published is **provider- and configuration-dependent**:
    /// the default is the empty state, exactly as other substrates default their state.
    /// Consumers must therefore treat any particular slot as optional and must not assume the
    /// state is non-empty.
    ///
    /// This is the extension point for emission-scoped metrics; provenance (who/where/what)
    /// lives in the typed [#subject()]/[#current()]/[#emission()] accessors.
    ///
    /// @return The per-emission measures, or an empty state when none are published
    /// @see State
    /// @see Name
    /// @since 2.10
    ...
    /// Returns the subject of the pipe this capture is attributed to, providing access to the
    /// pipe's name, ID, and state.
    ///
    /// Captures are produced by a [Sink], a pool of channels used like a [Conduit], so a capture is
    /// attributed to the **sink channel the value was emitted into** — the channel owns its emissions,
    /// exactly as a conduit pipe does. When a sink is bridged onto a source (via the pool-backed
    /// subscriber), the sink's channel mirrors the source channel's name but keeps its own identity.
    ///
    /// @return The subject of the pipe this capture is attributed to
    /// @see Subject
    /// @see Pipe
    /// @since 2.0
  ...
  /// A circuit-owned, initialized state cell with safe publication.
  ///
  /// A Cell stores the latest state accepted through the pipe returned by
  /// [#pipe()]. The cell itself is a [Substrate] with subject
  /// identity, but it is not a public [Receptor] or [Pipe]: callers cannot emit
  /// directly into it. Updates occur only when a circuit processes emissions
  /// sent through a pipe targeting the cell.
  ///
  /// Cells are useful when several channels, subscribers, or circuits need to
  /// update shared state in a deterministic circuit context while readers need a
  /// stable handle for the current value.
  ///
  /// ## Initialization (2.9)
  ///
  /// Cells are always initialized. They are created via [Circuit#cell(Object)]
  /// with a seed value, and [#get()] never returns `null`. The seed represents
  /// the cell's initial published value — typically the algebraic identity of
  /// whatever accumulation feeds the cell (zero for a counter, an empty set for
  /// a unique-collection, `Stats.empty()` for running statistics, and so on).
  /// Applications that need "no value has been published yet" semantics should
  /// model that as a domain value such as `Optional<E>` or an explicit sentinel.
  ///
  /// ## Safe Publication
  ///
  /// After an update has been processed by the owning circuit, subsequent reads
  /// that are ordered after that processing observe the published value. The
  /// publication guarantee applies to the value reference and to state reachable
  /// from that value at publication time. If `E` is mutable, callers must treat
  /// published values as immutable snapshots or provide their own synchronization;
  /// mutating a published `Map`, collection, array, or object is outside the
  /// cell's safety guarantee.
  ///
  /// Cells do not define transformation or accumulation semantics. Use
  /// [Fiber] or [Flow] to compute the value that should be published, then
  /// terminate that pipeline with [#pipe()] or pass the cell directly to
  /// [Fiber#pipe(Cell)] / [Flow#pipe(Cell)]. This keeps Cell as the end value
  /// and leaves temporal/stateful operators on the pipeline surfaces.
  ///
  /// @param <E> the type of state held by this cell
  /// @see Circuit#cell(Object)
  /// @see Fiber#pipe(Cell)
  /// @see Flow#pipe(Cell)
  /// @since 2.7
  ...
  @Provided
  @SpecRef ( {"11.2", "16.2"} )
  non-sealed interface Cell < E >
    extends Substrate < Cell < E > > {

    /// Returns the current value held by this cell.
    ///
    /// A cell is always initialized — created with a seed value via
    /// [Circuit#cell(Object)] — so `get()` never returns `null`. Each accepted
    /// emission through [#pipe()] replaces the published value with another
    /// non-null value.
    ///
    /// @return the current published value; never `null`
    /// @since 2.7
    ...
    /// Returns the pipe that updates this cell.
    ///
    /// The returned pipe is the only public update capability for this cell.
    /// Emissions are queued on the cell owner's circuit, then processed by the
    /// cell in that circuit's worker context. Repeated calls return the same
    /// pipe instance.
    ///
    /// @return a pipe that updates this cell
  ...
  /// An immutable snapshot of a change between two consecutive runs, produced by
  /// [Flow#change()] when an admission closes one run and opens the next. It
  /// carries the value the closed run held ([#from()]), the value that opened the
  /// next run ([#to()]), and the terminal [#length()] the closed run reached.
  ///
  /// Each change produces a fresh, immutable envelope — a pure
  /// out-of-execution-scope record, safe to retain, compare, and forward with no
  /// handle to any live counterpart (the [Capture] carrier pattern).
  ///
  ...
  interface Change < E > {
    ...
    /// Returns the value held by the run that just closed — the closed-run value
    /// before the change (this is that run's value, not its length; for the
    /// length see [#length()]).
    ///
    ...
    /// Returns the terminal length the closed run reached before the change — the
    /// length [Flow#run()] would have reported for the [#from()] value on the
    /// admission immediately before the change.
    ///
    /// Lengths use Java's signed 64-bit `long`. A run exceeding [Long#MAX_VALUE]
    /// is outside this projection's representable range; callers must not rely on
    /// wraparound as a valid run length.
    ///
    /// @return the closed run's length, always `>= 1`

    @SpecRef ( "6.2.3" )
    long length ();


    /// Returns the value that opened the new run — the admission that triggered
    /// the change.
    ///
  ...
  ///
  /// **A pipe is the capability to admit an emission** (the [Pipe] interface, with `emit()`);
  /// [Receptor] is the callback that receives one. The terms **"inlet"** and **"outlet"**
  /// describe **roles that pipes play relative to the circuit boundary**, not distinct types:
  ///
  ...
  ///
  /// ## Cascading Emission Ordering (Ingress and Transit)
  ///
  /// Queued work falls into two ordering classes with deterministic priority between them.
  /// These are roles, not a required data structure: a provider may realize them with two
  /// queues, one mailbox with priority markers, local slots, or actor turns (SPEC §5.3).
  ///
  /// - **Ingress work**: work submitted from any context other than this circuit's own —
  ///   external threads, and other circuits
  /// - **Transit work**: work raised from within this circuit's context during processing
  ///
  /// The class of a queued operation follows the context that submitted it, not the kind of
  /// operation it is (SPEC §5.3). Emissions are the usual case, but a subscription registration,
  /// a subscription close, or cleanup work raised from inside a callback joins transit on the
  /// same terms, and takes effect at its own position within the cascade.
  ///
  /// When processing an ingress item raises further work from within the circuit context, that
  /// work is **transit** work, and transit has **priority** over ingress:
  ///
  /// **Key characteristics**:
  /// - **No call stack recursion**: All emissions are staged rather than invoked as nested calls
  /// - **Stack safe**: Even deeply cascading chains don't overflow the stack
  /// - **Priority processing**: Transit drains completely before returning to ingress
  ...
  ///
  /// When transit work itself submits further work, that work is sequenced **behind transit work
  /// already accepted** for the current cascade (not recursively invoked), maintaining the
  /// iterative model. Transit is FIFO over every kind of operation it carries, not over
  /// emissions alone.
  ///
  ...
  @Tenure ( Tenure.EPHEMERAL )
  @Provided
  @SpecRef ( {"5.1", "9.3", "16.2"} )
  interface Circuit
    extends Resource < Circuit > {
    ...
    /// Returns a name-indexed bank of conduits with this circuit's default routing.
    ///
    /// The emission type is taken from the call site. Equivalent to `bank(Routing.PIPE)`.
    ///
    /// @param <E> the class type of emitted values
    /// @return A bank of conduits using this circuit for emission processing
    /// @throws Fault if this circuit has been closed
    ...
    /// Returns a name-indexed bank of conduits with the specified routing behavior.
    ///
    /// The emission type is taken from the call site. Use an explicit type argument where the
    /// context does not supply one, including where the emission type is itself parameterized
    /// and therefore cannot be named by a class literal.
    ///
    /// @param routing controls how emissions are dispatched within banked conduits
    /// @param <E>     the class type of emitted values
    /// @return A bank of conduits using this circuit for emission processing
    /// @throws NullPointerException if the specified routing is `null`
    /// @throws Fault                if this circuit has been closed
    /// @see Bank
    /// @see Routing
    /// @see #bank(Class, Routing)
    ...
    /// Returns a basin owned by this circuit that retains the most recent
    /// `capacity` values emitted through its [Basin#pipe()].
    ///
    /// A basin is the multi-value sibling of a [Cell]; see [Basin] for the
    /// buffering and read contract. The element type is recovered by target
    /// typing — assign the result to a typed variable. This form also serves
    /// nested element types such as `Basin<Capture<E>>`, which a class token
    /// cannot express.
    ///
    /// @param capacity the maximum number of recent values retained
    /// @param <E>      the type of value buffered by the basin
    /// @return a new basin owned by this circuit
    ...
    /// Returns a basin owned by this circuit, using the supplied class as a type
    /// witness for inference.
    ///
    /// Equivalent to [#basin(int)] with the element type taken from `type`. The
    /// class is used only for inference and is not retained. A class token cannot
    /// express a parameterized element type (for example `Capture<E>`); use
    /// [#basin(int)] with target typing for those.
    ///
    /// @param type     the class of the buffered value (for type inference only)
    /// @param capacity the maximum number of recent values retained
    /// @param <E>      the type of value buffered by the basin
    /// @return a new basin owned by this circuit
    ...
    default < E > Basin < E > basin (
      ...
      return basin ( capacity );

    }


    /// Returns a named basin owned by this circuit that retains the most recent
    /// `capacity` values emitted through its [Basin#pipe()].
    ///
    /// The supplied name is bound to the basin's subject; the one-argument form
    /// uses the owning circuit's name. The element type is taken from the call
    /// site, so this form also serves parameterized element types such as
    /// `Basin<Capture<E>>`, which a class token cannot express.
    ///
    /// @param name     the name given to the basin's subject
    /// @param capacity the maximum number of recent values retained
    /// @param <E>      the type of value buffered by the basin
    /// @return a new basin owned by this circuit
    /// @throws NullPointerException     if name is `null`
    ...
    /// Returns a named basin owned by this circuit, using the supplied class as a
    /// type witness for inference.
    ///
    /// Equivalent to [#basin(Name, int)] with the element type taken from `type`.
    /// The class is used only for inference and is not retained.
    ///
    /// @param name     the name given to the basin's subject
    /// @param type     the class of the buffered value (for type inference only)
    /// @param capacity the maximum number of recent values retained
    ...
    default < E > Basin < E > basin (
      ...
      return basin ( name, capacity );

    }


    /// Returns a circuit-owned cell seeded with an initial value.
    ///
    /// [Cell#get()] returns `initial` immediately, before any emission is
    /// processed. Each accepted emission through [Cell#pipe()] replaces the
    /// published value with another non-null value.
    ///
    /// [Cell#get()] may be called from any thread; it observes the value
    /// published by the most recent emission processed on the owning circuit
    /// before that read. The cell remains readable after the circuit closes;
    /// the published value is whatever was last accepted before shutdown.
    ///
    /// ## Initialization And Null Policy (2.9)
    ///
    /// Cells are always initialized. The 2.7 no-argument `cell()` factory has
    /// been removed in 2.9; callers that need "no value has been published yet"
    /// semantics should model that as a domain value, for example
    /// `Cell<Optional<E>>` or an explicit sentinel.
    ///
    /// The returned Cell's subject inherits the owning circuit's name. Use
    /// [#cell(Name, Object)] for an explicit subject name.
    ///
    /// @param initial the seed value, visible to readers before the first emission is accepted
    /// @param <E>     the type of state held by the cell
    /// @return a new seeded cell owned by this circuit
    ...
    /// Returns a named circuit-owned cell seeded with an initial value.
    ///
    /// Same semantics as [#cell(Object)] but binds the supplied name to the
    /// returned Cell's subject. The name must have been obtained from this
    /// runtime's [Cortex]; passing a name created by a different provider
    /// raises [Fault].
    ///
    /// @param name    the name assigned to the returned cell's subject
    /// @param initial the seed value, visible to readers before the first emission is accepted
    /// @param <E>     the type of state held by the cell
    /// @return A new named seeded cell owned by this circuit
    ...
    /// 3. **Closed**: The processing thread has terminated. Sources created from
    ///    the circuit ([Conduit]) are independent [Resource]s and should be
    ///    closed explicitly or managed by a [Scope] when deterministic source
    ///    cleanup is required.
    ///
    ...
    /// - Pure queued operations such as emissions and subscription close do not throw
    ///   synchronously merely because the receiver closes. Work admitted after the
    ///   component's close job is dropped when processed, subject to the operation's
    ///   callback symmetry. Providers document whether pre-close work drains or is dropped.
    ///   Callers that need to confirm no further operations will take effect use [#await()].
    /// - [Source#subscribe(Subscriber)] validates the source and subscriber synchronously
    ///   before admitting registration work. A closed source or subscriber causes [Fault]
    ///   on the caller thread. Once registration is accepted, its queued processing follows
    ///   the lifecycle rules above; this does not remove the initial open-state checks.
    /// - Synchronous `@New` factory operations —
    ///   [#conduit(Name, Class)], [#bank(Class)], [#subscriber(Name, BiConsumer)],
    ///   [#basin(int)] and friends — raise [Fault] on
    ...
    /// Returns a named conduit that will use this circuit to process and transfer values emitted.
    ///
    /// The emission type is taken from the call site. Use an explicit type argument where the
    /// context does not supply one, including where the emission type is itself parameterized
    /// and therefore cannot be named by a class literal:
    ///
    /// ```java
    /// var captures = circuit.<Capture<Integer>>conduit ( name );
    /// ```
    ///
    ...
    ///
    /// Convenience overload that uses the class parameter to drive type inference at the
    /// call site. Equivalent to [#conduit(Name)] with an explicit type witness.
    ///
    /// @param name the name given to the conduit's subject
    /// @param type the class type of emitted values (used for type inference)
    /// @param <E>  the class type of emitted values
    ...
    /// Returns a conduit with the specified routing behavior.
    ///
    /// The emission type is taken from the call site; the conduit takes this circuit's
    /// subject name.
    ///
    /// @param routing controls how emissions are dispatched within the conduit
    /// @param <E>     the class type of emitted values
    ...
    ///
    /// Convenience overload that uses the class parameter to drive type inference at the
    /// call site. Equivalent to [#conduit(Name, Routing)] with an explicit type witness.
    ///
    /// @param name    the name given to the conduit's subject
    /// @param type    the class type of emitted values (used for type inference)
    /// @param routing controls how emissions are dispatched within the conduit
    ...
    /// Returns a circuit-owned pin seeded with an initial value.
    ///
    /// A [Pin] is an immediate-access handle to circuit-owned state, guarded by
    /// the owning circuit context. Unlike [#port(Object)], which is queued and
    /// exposes no read accessor, a Pin's [Pin#get()] and [Pin#set(Object)]
    /// execute inline — but only when invoked from the owning circuit's
    /// processing thread. Invocation from any other context raises
    /// [IllegalStateException].
    ///
    /// Use Pin when circuit-local code needs "set X, then read X on the next
    /// line" semantics, or when mutable in-place objects must be updated
    /// under circuit-context confinement. Pin handles MAY be emitted as values
    /// — the receiver may store or forward the handle but cannot dereference
    /// it from outside the owning circuit.
    ///
    /// The returned Pin's subject inherits the owning circuit's name. Use
    /// [#pin(Name, Object)] for an explicit subject name.
    ///
    /// @param initial the seed value
    /// @param <E>     the type of state held by the pin
    /// @return A new seeded pin owned by this circuit
    /// @throws NullPointerException if initial is `null`
    /// @throws Fault                if this circuit has been closed
    /// @see Pin
    /// @see #pin(Name, Object)
    ...
    /// Returns a named circuit-owned pin seeded with an initial value.
    ///
    /// Same semantics as [#pin(Object)] but binds the supplied name to the
    /// returned Pin's subject. The name must have been obtained from this
    /// runtime's [Cortex]; passing a name created by a different provider
    /// raises [Fault].
    ///
    /// @param name    the name assigned to the returned pin's subject
    /// @param initial the seed value
    /// @param <E>     the type of state held by the pin
    /// @return A new named seeded pin owned by this circuit
    /// @throws NullPointerException if any argument is `null`
    /// @throws Fault                if the name parameter is not a runtime-provided implementation, or if this circuit has been closed
    /// @see Pin
    /// @see #pin(Object)
    ...
    /// Returns a pipe that queues emissions on this circuit and discards them,
    /// using the supplied class as a type witness for inference.
    ///
    /// Equivalent to [#pipe()] with the emission type taken from `type`. The class
    /// is used only for inference and is not retained, so a parameterized emission
    /// type — which no class literal can name — is served by [#pipe()] with an
    /// explicit type argument or target typing.
    ///
    /// @param type the class of the emission type (for type inference only)
    /// @param <E>  the class type of emitted values
    /// @return A pipe that queues emissions on this circuit and discards them
    /// @throws NullPointerException if the specified type is `null`
    /// @throws Fault                if this circuit has been closed
    /// @see Pipe
    /// @see #pipe()
    /// @see Receptor#of(Class)
    /// @since 3.1.0

    @New
    @NotNull
    @SpecRef ( {"5.3", "6.1", "14"} )
    default < E > Pipe < E > pipe (
      @NotNull final Class < E > type
    ) {
    ...
    /// Returns a named pipe that queues emissions on this circuit and discards them.
    ///
    /// The supplied name is bound to the returned pipe's subject; [#pipe()] uses the
    /// owning circuit's name. Because the name MUST be honored, this always mints a
    /// new circuit-owned pipe.
    ///
    /// @param name the name given to the pipe's subject
    /// @param <E>  the class type of emitted values
    /// @return A named pipe that queues emissions on this circuit and discards them
    /// @throws NullPointerException if the specified name is `null`
    /// @throws Fault                if the name parameter is not a runtime-provided implementation, or if this circuit has been closed
    /// @see Pipe
    /// @see #pipe()
    ...
    /// using the supplied class as a type witness for inference.
    ///
    /// Equivalent to [#pipe(Name)] with the emission type taken from `type`. The class
    /// is used only for inference and is not retained.
    ///
    /// @param name the name given to the pipe's subject
    /// @param type the class of the emission type (for type inference only)
    /// @param <E>  the class type of emitted values
    ...
    /// Returns a pipe that admits emissions through this circuit before forwarding
    /// them to `target`.
    ///
    /// Use this as a circuit hop: the returned pipe is owned by this circuit, so
    /// calls to [Pipe#emit(Object)] are queued on this circuit. When this circuit
    /// processes the emission, it forwards the value to `target`; from that point
    /// the target pipe keeps its own normal queuing, ordering, and circuit-ownership
    /// semantics.
    ///
    /// This method does not add a special recursion guard. Pipe emission is already
    /// queued by the substrate model, including cascades from circuit-thread work.
    /// This overload only chooses the ingress circuit for an existing target pipe.
    ///
    /// ## Dispatch
    ///
    /// For cross-circuit targets, each emission first runs as work on this circuit
    /// and is then admitted to the target pipe's circuit. This creates an explicit
    /// queue-to-queue boundary, useful for staging pipelines across circuits or
    /// shifting caller ingress away from the target circuit.
    ///
    /// ## Same-Circuit Optimization
    ///
    /// If the target pipe already belongs to this circuit, it is returned as-is
    /// without wrapping. No additional pipe, queue hop, or allocation is added.
    ///
    /// The target follows the same contravariance as [Registrar#register(Pipe)]:
    /// a `Pipe<Number>` target can receive `Integer` emissions, so the returned
    /// pipe may be typed to the narrower emission type.
    ///
    /// @param target the downstream pipe that receives forwarded emissions
    /// @param <E>    the class type of emitted values
    /// @return A pipe whose ingress is this circuit and downstream is `target`, or the target itself if already on this circuit
    /// @throws NullPointerException if the target is `null`
    /// @throws Fault                if the target parameter is not a runtime-provided implementation, or if this circuit has been closed
    /// @see Pipe
    ...
    /// Returns a pipe owned by this circuit that fans out emissions to a fixed
    /// list of target pipes.
    ///
    /// Each emission is forwarded to the targets in list order. Same-circuit
    /// targets observe that order on this circuit; cross-circuit targets are
    /// enqueued in list order but execute on their own circuits' workers, so
    /// completion order across circuits is not guaranteed. The targets are
    /// snapshotted at creation time; later mutations to the caller's list have
    /// no effect on the returned pipe.
    ///
    /// ## Contract
    ///
    /// - Null list or any null element is rejected synchronously.
    /// - Targets are dispatched in list order; duplicate entries cause the
    ///   target to receive the emission once per entry.
    /// - An empty list returns a no-op pipe identical to [#pipe()].
    /// - A single-target list is equivalent to [#pipe(Pipe)].
    /// - The returned pipe is owned by this circuit; every emission enters
    ///   this circuit's queue before fan-out dispatch.
    ///
    /// @param targets the pipes to receive each emission, in order
    /// @param <E>     the emission type
    /// @return A pipe that fans out to all targets
    /// @throws NullPointerException if targets is null or contains a null element
    /// @throws Fault                if any target is not a runtime-provided implementation, or if this circuit has been closed
    /// @see #pipe(Pipe)
    /// @since 3.0
    ...
    @SpecRef ( {"5.3", "6.1", "14"} )
    < E > Pipe < E > pipe (
      @NotNull List < ? extends Pipe < ? super E > > targets
    );


    /// Returns a named pipe owned by this circuit that fans out emissions to a
    /// fixed list of target pipes.
    ///
    /// The name is assigned to the returned pipe's [Subject], making the fan-out
    /// pipe identifiable in subscriber callbacks, subject hierarchies, and diagnostic
    /// tooling. This is the named form of [#pipe(List)] and shares its fan-out
    /// contract — list-order dispatch, per-occurrence duplicates, and creation-time
    /// snapshotting. The one difference: because the supplied name must be honored,
    /// this overload always mints a new named pipe rather than collapsing the
    /// degenerate sizes — an empty list yields a named no-op pipe, and a
    /// single-target list yields a named pipe forwarding to that target. Use
    /// `pipe(name, List.of(target))` when a named single-target forwarder is
    /// required.
    ///
    /// The name must have been obtained from this runtime's [Cortex]; passing a
    /// name created by a different provider throws [Fault].
    ///
    /// @param name    the name assigned to the returned pipe's subject
    /// @param targets the pipes to receive each emission, in order
    /// @param <E>     the emission type
    /// @return A named pipe that fans out to all targets
    /// @throws NullPointerException if name or targets is null, or targets contains a null element
    /// @throws Fault                if name or any target is not a runtime-provided implementation, or if this circuit has been closed
    /// @see #pipe(List)
    /// @since 3.0
    ...
    /// Returns a pipe owned by this circuit that delivers emissions to a receptor.
    ///
    /// Calls to [Pipe#emit(Object)] are queued on this circuit. The receptor is
    /// invoked on the circuit's processing thread, ensuring sequential execution
    /// and deterministic ordering for receptor-side effects.
    ///
    /// @param receptor the receptor invoked on this circuit's processing thread
    /// @param <E>      the class type of emitted values
    /// @return A circuit-owned pipe that delivers queued emissions to the receptor
    ...
    /// Returns a named pipe owned by this circuit that delivers emissions to a
    /// receptor.
    ///
    /// The name is assigned to the returned pipe's [Subject], making the pipe
    /// identifiable in subscriber callbacks, subject hierarchies, and diagnostic
    /// tooling. Emission semantics, sequencing, and exception isolation are
    /// identical to [#pipe(Receptor)].
    ///
    /// The name must have been obtained from this runtime's [Cortex]; passing a
    /// name created by a different provider throws [Fault].
    ///
    ...
    /// @param receptor the receptor invoked on this circuit's processing thread
    /// @param <E>      the class type of emitted values
    /// @return A named circuit-owned pipe that delivers queued emissions to the receptor
    /// @throws NullPointerException if any argument is `null`
    /// @throws Fault                if name is not from this runtime provider, or if this circuit has been closed
    /// @see #pipe(Receptor)
    /// @since 2.7
    ...
    /// Returns a circuit-owned port seeded with an initial value.
    ///
    /// A [Port] is a queued access boundary for circuit-owned state. It grants
    /// the holder transform/replace/emit authority without exposing direct
    /// read access — transformations execute inside the owning circuit context
    /// and the current value is never observable to the Port's holder.
    ///
    /// The supplied initial value seeds the Port; transformations passed to
    /// [Port#update(UnaryOperator)] receive this value on first invocation.
    /// Ports are always initialized (§6 of the 2.9 design); there is no
    /// no-argument port factory.
    ///
    /// The returned Port's subject inherits the owning circuit's name. Use
    /// [#port(Name, Object)] for an explicit subject name.
    ///
    /// @param initial the seed value
    /// @param <E>     the type of state held by the port
    /// @return A new seeded port owned by this circuit
    /// @throws NullPointerException if initial is `null`
    /// @throws Fault                if this circuit has been closed
    /// @see Port
    /// @see #port(Name, Object)
    /// @since 2.9
    ...
    /// Returns a named circuit-owned port seeded with an initial value.
    ///
    /// Same semantics as [#port(Object)] but binds the supplied name to the
    /// returned Port's subject. The name must have been obtained from this
    /// runtime's [Cortex]; passing a name created by a different provider
    /// raises [Fault].
    ///
    /// @param name    the name assigned to the returned port's subject
    /// @param initial the seed value
    /// @param <E>     the type of state held by the port
    /// @return A new named seeded port owned by this circuit
    /// @throws NullPointerException if any argument is `null`
    /// @throws Fault                if the name parameter is not a runtime-provided implementation, or if this circuit has been closed
    /// @see Port
    /// @see #port(Object)
    ...
    /// Returns a sink that funnels emissions, each enriched as a [Capture], into the
    /// given endpoint pipe.
    ///
    /// A [Sink] is the output-closed dual of [Conduit]: a [Pool] of [Pipe] of `E` whose
    /// destination is fixed at creation rather than discovered by subscription. Each value
    /// emitted into one of the sink's named pipes is wrapped in a [Capture] — bearing the
    /// emitting pipe's [Capture#subject()], the [Capture#current()] emitting context, the
    /// [Capture#emission()] value, and any provider [Capture#state()] — and forwarded to the
    /// endpoint.
    ///
    /// The emission type `E` is inferred from the endpoint's [Capture] type, so no type
    /// witness is required. The endpoint need not belong to this circuit. Uses this circuit's
    /// subject name for the sink; for an explicit name use [#sink(Name, Pipe)].
    ///
    /// @param endpoint the pipe that receives a capture for each emission
    /// @param <E>      the class type of emitted values wrapped into captures
    /// @return A sink with the circuit's name, funneling captures into the endpoint
    /// @throws NullPointerException if endpoint is `null`
    /// @throws Fault                if the endpoint is not a runtime-provided implementation, or if this circuit has been closed
    /// @see Sink
    /// @see Capture
    /// @see #sink(Name, Pipe)
    /// @since 2.10
    ...
    @SpecRef ( {"10.5", "11.1"} )
    < E > Sink < E > sink (
      @NotNull Pipe < Capture < E > > endpoint
    );


    /// Returns a named sink that funnels emissions, each enriched as a [Capture], into the
    /// given endpoint pipe.
    ///
    /// Same semantics as [#sink(Pipe)] but binds the supplied name to the returned sink's
    /// subject. The name must have been obtained from this runtime's [Cortex]; passing a name
    /// created by a different provider raises [Fault].
    ///
    /// @param name     the name assigned to the returned sink's subject
    /// @param endpoint the pipe that receives a capture for each emission
    /// @param <E>      the class type of emitted values wrapped into captures
    /// @return A named sink funneling captures into the endpoint
    /// @throws NullPointerException if any argument is `null`
    /// @throws Fault                if the name or endpoint is not a runtime-provided implementation, or if this circuit has been closed
    /// @see Sink
    /// @see Capture
    /// @see #sink(Pipe)
    /// @since 2.10
    ...
    /// unaffected. Registration calls completed before the failure remain registered.
    /// The callback is considered consumed and MUST NOT be retried for that
    /// subscription/channel pair. The failure remains observable through the provider's
    /// documented failure-reporting mechanism (see §15.4 invariant 3).
    ///
    /// @param <E>      the emission type
    /// @param name     the name to be used by the subject assigned to the subscriber
    /// @param behavior the subscribing behavior to be applied when a channel receives its first emission after subscription
    /// @return A new subscriber scoped to this circuit
    /// @throws NullPointerException if the name or behavior is `null`
    /// @throws Fault                if the name parameter is not a runtime-provided implementation, or if this circuit has been closed
    ...
    < E > Subscriber < E > subscriber (
      @NotNull Name name,
      @NotNull @Queued BiConsumer < ? super Subject < Pipe < E > >, ? super Registrar < E > > behavior
    );
    ...
    /// ```
    ///
    /// Because [Conduit] and [Sink] both extend [Pool] of [Pipe], either may be
    /// passed directly: a conduit for self-feedback wiring, or a sink to funnel each
    /// discovered channel into a fixed capture endpoint.
    ///
    /// ```java
    /// Sink<Integer> sink = circuit.sink(endpoint);
    /// source.subscribe(circuit.subscriber(name, sink));
    /// ```
    ///
    ...
    /// Schedules periodic emissions of monotonically increasing sequence
    /// numbers into the target pipe, with each emission processed through
    /// this circuit's queue.
    ///
    /// A ticker submits a tick job carrying the next sequence number to this
    /// circuit once per interval; how a provider schedules that submission is
    /// its own affair. The first emission carries sequence `0`; each subsequent emission
    /// increments by one. Sequence numbers carry no wall-clock semantics —
    /// receptors can detect missed or delayed ticks from gaps or stalls in
    /// the sequence.
    ///
    /// ## Scheduling
    ///
    /// The scheduling cadence is fixed-rate: ticks target points on a grid,
    /// each one `interval` after the last. A submission that wakes late
    /// does not shift the grid — the next wait is shortened to compensate —
    /// so scheduling error does not accumulate and the average interval
    /// over a run converges to `interval`.
    ///
    /// Catch-up is bounded. If a submission wakes more than one `interval`
    /// past its grid point — for example after a long garbage-collection
    /// pause — the ticker re-anchors the grid to the current time rather
    /// than firing the skipped grid points back-to-back. A stall therefore
    /// produces a one-time phase shift, never a burst; at most one tick may
    /// fire early to rejoin the grid.
    ///
    /// Submission is non-blocking — the ticker hands the emission to the
    /// circuit and returns to await the next grid point. The grid governs
    /// submission timing only; latency added by the circuit queue between
    /// submission and delivery is not part of this guarantee.
    ///
    /// ## Lifecycle
    ///
    /// The returned [Ticker] controls the ticker and should be closed
    /// explicitly or managed by a [Scope]:
    /// - [Ticker#close()] signals the ticker to stop; pending in-flight
    ///   emissions may still be delivered to the target before cleanup
    ///   completes
    /// - Closing the circuit rejects future tick submissions, but does not
    ///   retain or close the ticker handle
    ///
    /// ## Threading
    ///
    /// Each tick first enters this circuit's queue. When the tick job runs,
    /// it forwards to the target pipe through normal pipe routing:
    /// same-circuit targets continue through worker-local transit ordering,
    /// while cross-circuit targets enqueue onto their owning circuit.
    ///
    /// ## Failure Isolation
    ///
    /// Exceptions thrown by the target receptor are isolated by the circuit
    /// that executes the target and do not stop the ticker. The ticker only
    /// stops when closed, or when its owning circuit rejects future tick
    /// submissions during shutdown.
    ///
    /// ## Backpressure
    ///
    /// If the target receptor is slower than `interval`, queued ticker work
    /// can accumulate. Callers that want to coalesce, sample, or rate-limit
    /// when behind should compose the
    /// target with a [Fiber] (`every`, `diff`, `reduce`) rather than
    /// expecting the ticker to drop ticks.
    ///
    /// @param interval the duration between successive ticks; must be positive
    /// @param target   the pipe to receive tick sequence emissions
    /// @return A ticker handle that stops the scheduled emissions when closed
    /// @throws NullPointerException     if any argument is `null`
    /// @throws IllegalArgumentException if interval is zero or negative
    /// @throws Fault                    if this circuit has been closed
    /// @see #ticker(Name, Duration, Pipe)
    /// @see Ticker
    /// @since 2.8
    ///
    /// Java [Duration] supplies nanosecond precision. Its own construction and
    /// arithmetic overflow rules apply before this operation is invoked; this API
    /// adds no ticker-specific overflow conversion or rounding semantics.
    ...
    Ticker ticker (
      ...
      @NotNull final Pipe < ? super Long > target
    );


    /// Schedules periodic emissions of monotonically increasing sequence
    /// numbers into the target pipe with an explicit ticker name.
    ///
    /// The returned ticker carries `name` as its subject name, allowing
    /// multiple tickers on a circuit to be distinguished in logs, fault
    /// reports, and observability dashboards. Semantics are otherwise
    /// identical to [#ticker(Duration, Pipe)].
    ///
    /// @param name     the name assigned to the ticker's subject
    /// @param interval the duration between successive ticks; must be positive
    /// @param target   the pipe to receive tick sequence emissions
    /// @return A ticker handle that stops the scheduled emissions when closed
    /// @throws NullPointerException     if any argument is `null`
  ...
  ///
  /// The Java projection requires callers to treat a Closure as consumed after its first
  /// [#consume(Consumer)] invocation. It does not require providers to signal repeated use: except
  /// for the owning-Scope-closed no-op specified above, invoking `consume` again has undefined
  /// behavior under SPEC §6.4.1. Providers may detect and reject reuse as a stronger diagnostic.
  ///
  ...
  /// - **Source**: Emits events and allows subscription to channels, with subscriber
  ///   callbacks invoked lazily on a channel's first emission
  ///
  ...
  /// - Different names create different pipes
  /// - Each pipe has its own subject, and plays the channel role for that name
  ///
  /// **Identity guarantee**: Within this conduit, repeated lookup of the same name returns
  /// the same pipe instance. This guarantee does not equate pipes from different conduits.
  ///
  ...
  /// - First call to [Pool#get(Name)] creates the named pipe (no callbacks invoked)
  /// - First emission through that pipe triggers rebuild on circuit thread
  ...
  /// circuits, currents, fibers, flows, names, pools, scopes, slots, and states. Each component type
  ...
  non-sealed interface Cortex
    ...
    /// [Fiber#pipe(Cell)] or wrap as a Flow via [#flow(Fiber)].
    ///
    /// @param <E> the emission type
    ...
    /// [Flow#pipe(Pipe)] / [Flow#pipe(Cell)]. The emission type is inferred from the
    ...
    /// @throws IllegalArgumentException if the path is empty, begins or ends with `.`, contains
    ///                                  consecutive separators (empty segments), or would exceed
    ///                                  the implementation's maximum depth (see [Name])
    ...
    /// @throws IllegalArgumentException if the iterable yields no values, yields an invalid path
    ///                                  segment, or the resulting name would exceed the
    ///                                  implementation's maximum depth (see [Name])
    ...
    /// @throws IllegalArgumentException if the iterator yields no values, yields an invalid path
    ///                                  segment, or the resulting name would exceed the
    ///                                  implementation's maximum depth (see [Name])
    /// @see #name(Iterable)
    ...
    /// Returns a root pool that materializes values on demand from names.
    ///
    /// The returned pool invokes the factory function **exactly once per name**;
    /// the cached result (or failure) is then replayed for subsequent lookups of
    /// the same name — the same contract as derived pools created via
    /// [Pool#pool(Function)]. Retrieval is thread-safe per the [Lookup] contract.
    ///
    /// A root pool composes with the rest of the pool machinery: derived views
    /// via [Pool#pool(Function)] and pool-backed subscribers via
    /// [Circuit#subscriber(Name, Pool)]. For example, name-keyed circuit state:
    ///
    /// ```java
    /// Pool<Cell<Integer>> cells = cortex.pool(name -> circuit.cell(name, 0));
    /// source.subscribe(circuit.subscriber(name, cells.pool(Cell::pipe)));
    /// int latest = cells.get(channel).get();
    /// ```
    ///
    /// ## Function Contract
    ///
    /// The function MUST return a non-null result. If it returns `null`, the
    /// pool's [Lookup#get(Name)] raises [NullPointerException] for that name.
    ...
    /// every subsequent lookup of the same name without re-invoking the
    /// function.
    ///
    /// ## No Ownership
    ///
    /// The pool holds materialized values for lookup but does not own them:
    /// unlike a [Bank], there is no close, and no lifecycle is cascaded to
    /// materialized values. Use [Circuit#bank(Class)] when materialized
    /// [Resource]s must be closed with their holder.
    ///
    /// @param fn  the factory function invoked once per name (must return non-null for every name)
    /// @param <T> the type of values returned by the pool
    /// @return A root pool that caches materialized values by name
    /// @throws NullPointerException if fn is `null`
    /// @see Pool#pool(Function)
    /// @see Circuit#subscriber(Name, Pool)
    /// @see Bank
    /// @since 3.0
    ...
    @SpecRef ( {"10.1", "12"} )
    < T > Pool < T > pool (
      @NotNull Function < ? super Name, ? extends T > fn
    );
  ...
  interface Extent < S extends Extent < S, P >, P extends Extent < ?, P > >
    ...
    ///
    /// The initializer is applied exactly once, to the [#extremity()]; the accumulator is then
    /// applied once per remaining extent, moving toward `this`. A `null` returned by the
    /// initializer or the accumulator is not treated as absence and does not end the traversal:
    /// it is passed on as the accumulator input for the next extent, and a `null` returned by the
    /// final application becomes the result. This matches [#fold].
    ///
    /// This default implementation traverses iteratively, so its own depth is bounded by heap
    /// rather than by stack. An overriding implementation may traverse differently and impose its
    /// own bound.
    ///
    /// @param <R>         the return type of the accumulated value
    ...
    default < R > R foldTo (
      ...
      for ( var index = root - 1; index >= 0; index-- ) {
    ...
    /// Returns a fresh spliterator over the extents moving from the right (`this`)
    /// to the left ([#extremity()]).
    ///
    /// The spliterator reports [Spliterator#ORDERED] and [Spliterator#NONNULL].
    /// It deliberately reports neither [Spliterator#SIZED], [Spliterator#DISTINCT]
    /// nor [Spliterator#IMMUTABLE]. Sizing would cost a second full traversal;
    /// `Extent` does not require two levels of an enclosure chain to be unequal
    /// under `equals`; and structural immutability is not part of the `Extent`
    /// contract.
    ///
    /// @return A spliterator for traversing the nested extents from this extent to the root

    @New
    @NotNull
    @Override
    default Spliterator < P > spliterator () {
    ...
    ///
    /// The stream uses [#spliterator()], so the encounter order holds under
    /// `parallel()` as well as sequentially. It is lazy: a short-circuiting
    /// operation walks only as far into the enclosure chain as it needs.
    ///
  ...
  /// `change`, `chance`, `delay`, `diff`, `distinct`, `dropWhile`, `edge`, `every`,
  /// `guard(initial, BiPredicate)`, `heartbeat`, `high`, `hysteresis`, `inhibit`,
  /// `integrate`, `limit`, `low`, `pulse`, `reduce`, `relate`, `rolling`, `skip`,
  /// `steady`, `streak`, `takeWhile`, `tumble`.
  ///
  /// **Stateless operators** process each emission independently:
  /// `above`, `below`, `clamp`, `deadband`, `guard(Predicate)`, `max`, `min`,
  /// `peek`, `range`, `replace`, `route`, `tee`. `when` holds no state of its own —
  /// it carries whatever the sub-fiber it branches into holds.
  ///
  ...
  /// ## Lifecycle
  ///
  /// Fibers are **standalone immutable values**. Each operator returns an
  /// immutable Fiber representing the extended chain; parents are not mutated. A
  /// runtime may return an equivalent existing instance rather than a distinct one
  /// (see [New]), so callers must not rely on reference identity. Values obtained
  /// from [Cortex#fiber()] / [Cortex#fiber(Class)] may be composed via fluent
  /// chaining, retained, shared, and attached to many sinks
  /// ([Fiber#pipe(Pipe)] / [Fiber#pipe(Cell)], or wrapped in a [Flow] via
  /// [Cortex#flow(Fiber)] or [Flow#fiber(Fiber)]). Execution state for stateful operators is
  ...
  /// - Failure reporting follows the provider's documented mechanism. SPEC §15.4 permits
  ///   providers to offer a structured fault receptor; when supplied, it must receive isolated
  ///   callback failures with the original cause. Failures of that receptor are isolated
  ///   without recursive reporting. The current [Cortex] factories expose no such parameter;
  ///   consult the provider's default reporting behavior.
  ///
  ...
  interface Fiber < E > {
    ...
    /// Resolves [Comparator#naturalOrder()] against a pair of probe values so that
    /// the natural-ordering default methods fail fast at chain-build time — via
    /// [ClassCastException] — when the supplied bounds cannot be compared. The
    /// probe evaluates `a.compareTo(b)`.
    ///
    /// ## Scope of the guarantee
    ///
    /// The probe exercises the **runtime types of the supplied bounds**. It is not
    /// a check on the declared type parameter `E`, and it is not a check on future
    /// emissions:
    ///
    /// - A declared `E` that is not [Comparable] goes undetected whenever the bound
    ///   itself is comparable. `Fiber<Object>` with an `Integer` bound builds
    ///   without complaint.
    /// - The comparison is evaluated in one direction only. Two-bound callers probe
    ///   `lower.compareTo(upper)`; a `lower` whose `compareTo` accepts `upper` passes
    ///   even where `upper.compareTo(lower)` would throw.
    /// - An emission whose runtime type is incompatible with the bounds still fails
    ///   at emission time, not here.
    ///
    /// Full chain-build validation of arbitrary future `E` values is not expressible
    /// against the unbounded `Fiber<E>` type, so this probe is deliberately partial.
    ///
    /// Single-bound callers ([#above], [#below], [#max], [#min]) pass the same value
    /// for both arguments, reducing to `bound.compareTo(bound)`, which surfaces a
    /// non-[Comparable] **bound** at the call site.
    ///
    /// Two-bound callers ([#clamp], [#deadband], [#range]) pass `lower` and `upper`,
    /// which additionally establishes that `lower` accepts `upper` as an operand.
    ///
    ...
    /// @throws ClassCastException   if `a` is not [Comparable] or does not accept
    ///                              `b` as an operand
    /// @since 2.3
    ...
    /// [ClassCastException] is raised here — not at a later emission — if the
    /// supplied bound is not [Comparable]. The probe establishes nothing about the
    /// declared `E`, and does not prevent a later emission of an incompatible
    /// runtime type from failing at emission time.
    ///
    /// @param lower the exclusive lower bound
    /// @return A fiber that filters values at or below the lower bound
    /// @throws NullPointerException if lower is `null`
    /// @throws ClassCastException   if the supplied bound is not [Comparable]
    ...
    /// The bounds are probed as `lower.compareTo(upper)` at chain-build time, so a
    /// [ClassCastException] is raised here — not at a later emission — if `lower`
    /// is not [Comparable] or does not accept `upper` as an operand. The probe
    /// establishes nothing about the declared `E`, does not test the reverse
    /// direction, and does not prevent a later emission of an incompatible runtime
    /// type from failing at emission time.
    ///
    /// @param lower the inclusive lower bound
    /// @param upper the inclusive upper bound
    /// @return A fiber that clamps values to the specified range
    /// @throws NullPointerException     if any argument is `null`
    /// @throws ClassCastException       if `lower` is not [Comparable] or does
    ///                                  not accept `upper` as an operand
    ...
    /// Returns a fiber that samples values by elapsed processing time. The first
    /// observed value anchors the interval and is dropped; a later value observed
    /// after at least `duration` has elapsed is emitted. If more than one
    /// interval elapsed before that value arrives, the internal clock advances by
    /// the elapsed interval slots rather than by the late arrival time, avoiding
    /// drift after overruns. Stateful per materialization.
    ///
    /// This is a processing-time sampler: elapsed time is measured when the
    /// emission reaches this operator on the circuit worker, not when the value
    /// was originally submitted by the caller. Multiple time-aware operators
    /// processing the same queued emission may share a provider-cached worker
    /// timestamp.
    ///
    /// @param duration the minimum elapsed processing time between emitted values
    /// @return A stateful fiber with trailing-edge processing-time interval sampling
    /// @throws NullPointerException     if duration is `null`
    /// @throws IllegalArgumentException if duration is zero, negative, or cannot be represented in nanoseconds
    /// @since 2.7
    ...
    /// Returns a fiber that drops consecutive duplicates, but passes a duplicate
    /// through as a heartbeat once a run of unchanged values has persisted for at
    /// least `maxSilence` processing time. Stateful.
    ///
    /// A changed value (one that differs from the previously emitted value per
    /// [Objects#equals], as with [#diff()]) always passes and clears the silence
    /// timer. While a run of duplicates continues, the **first** duplicate anchors
    /// the timer and is dropped; a later duplicate observed at least `maxSilence`
    /// after that anchor passes as a heartbeat (re-emitting the held value
    /// unchanged) and re-anchors the timer. The first emission always passes.
    ///
    /// Silence is therefore measured from when the current run of duplicates was
    /// first observed, not from the last emitted value: the processing-time clock
    /// is consulted **only** on duplicate emissions, so a stream of changing
    /// values costs no more than [#diff()]. A heartbeat means "this value has been
    /// confirmed stable for `maxSilence`".
    ///
    /// Elapsed time is measured when the emission reaches this operator on the
    /// circuit worker (processing time), not when the value was submitted by the
    /// caller — the same clock used by [#every(Duration)]. Multiple time-aware
    /// operators processing the same queued emission may share a provider-cached
    /// worker timestamp.
    ///
    /// @param maxSilence the processing time a run of duplicates must persist
    ///                   before a duplicate is passed through as a heartbeat
    /// @return A stateful fiber that suppresses consecutive duplicates with a
    ///         max-silence heartbeat
    /// @throws NullPointerException     if maxSilence is `null`
    /// @throws IllegalArgumentException if maxSilence is zero, negative, or cannot
    ...
    ///
    /// The target follows the same contravariance as [Registrar#register(Pipe)]:
    /// a `Pipe<Number>` target can receive this fiber's `Integer` emissions.
    ///
    ...
    /// Returns a pipe that applies this fiber's operator chain ahead of the
    /// supplied cell's update pipe.
    ///
    /// Emissions that survive the fiber are forwarded to the cell's update pipe
    /// and become its published value. Filtered emissions do not update the cell.
    ///
    /// The cell is used strictly as a consumer — only its update pipe is
    /// attached — so it follows the same contravariance as [#pipe(Pipe)]:
    /// a `Cell<Number>` can publish this fiber's `Integer` emissions.
    ///
    /// @param cell the cell whose published value is updated by surviving emissions
    /// @return A pipe that applies this fiber before publishing to `cell`
    /// @throws NullPointerException if cell is `null`
    /// @throws Fault                if the cell is not a runtime-provided implementation
    /// @see Cell#pipe()
    /// @see #pipe(Pipe)
    /// @since 2.7
    ...
    ///
    /// Prefer this over `peek(pipe::emit)` when the intent is explicit
    ...
    ///
    /// Multiple `when` stages compose independently: a value may match several
  ...
  /// - [#scan(Supplier, BiFunction)] — emit the running state itself (`O → S`)
  ...
  /// - [#relate(Object, BiFunction)] — append a fold over consecutive pairs (`(O,O) → P`)
  /// - [#window(int)] — append a bounded rolling [Window] view at the output side
  /// - [#run()] — turn each admission into a [Run] carrying its run length
  /// - [#change()] — emit a [Change] at each run boundary (from value → to value, with length)
  ...
  /// new downstream type `P`, producing a `Flow<I, P>`. [#scan(Supplier, BiFunction)]
  /// appends a stateful fold whose state is the downstream value. [#scan(Supplier, BiFunction, Function)]
  /// appends the general stateful type-changing fold at the same output side; later
  /// stages see the scan output type `S` or `P`. [#window(int)] appends a stateful
  /// bounded rolling window at the output side, producing temporal `Window<O>` views.
  ///
  ...
  /// ## Lifecycle
  ///
  /// Flow instances are **standalone immutable values**. Each operator returns an
  /// immutable Flow representing the extended pipeline; parents are not mutated. A
  /// runtime may return an equivalent existing instance rather than a distinct one
  /// (see [New]), so callers must not rely on reference identity. Values obtained
  ...
  /// an attached Fiber, by [#scan(Supplier, BiFunction)], by
  /// [#scan(Supplier, BiFunction, Function)], or by [#window(int)] is allocated at
  /// materialization time, so a single Flow value can be materialized repeatedly to
  /// produce independent chains each with their own state slots.
  ///
  ...
  /// - Failure reporting follows the provider mechanism and the conditional structured-reporting
  ///   requirements described in [Fiber].
  ///
  ...
  interface Flow < I, O > {
    ...
    /// Returns a flow that emits a [Change] only at a run boundary — when an
    /// admission is value-unequal to its predecessor. The [Change] carries the
    /// value of the run that just closed ([Change#from()]), the value opening the
    /// next run ([Change#to()]), and the terminal length the closed run reached
    /// ([Change#length()]). The first admission opens the first run and emits
    /// nothing; the open or final run is never reported — its length is observable
    /// only through [#run()]. Value equality is [Objects#equals]; for changes over
    /// a derived key, [#map(Function)] to the key first.
    ///
    /// The emitted [Change] is a fresh immutable envelope — safe to retain,
    /// compare, and forward.
    ///
    /// ```
    /// cortex.flow ( Sign.class )
    ///   .change ()
    ///   .map ( c -> c.from () + " -> " + c.to () + " (" + c.length () + ")" );
    /// ```
    ///
    /// @return A stateful flow emitting a [Change] of `O` at each run boundary
    /// @see #run()
    /// @see Change
    /// @since 3.0
    ...
    ///
    /// This is the cross-type attachment point for Flow. The supplied pipe must
    /// be able to consume the flow's output type `O` — it follows the same
    /// contravariance as [Registrar#register(Pipe)], so a `Pipe<Number>` accepts
    /// a flow producing `Integer` — and the returned pipe carries the flow's
    /// input type `I`.
    ///
    /// ## Usage Patterns
    ///
    ...
    @SpecRef ( "6.2.6" )
    Pipe < I > pipe (
      @NotNull Pipe < ? super O > pipe
    );


    /// Returns a pipe that applies this flow's pipeline ahead of the supplied
    /// cell's update pipe.
    ///
    /// Output emissions from the flow are forwarded to the cell's update pipe
    /// and become its published value. Null outputs from the flow do not update
    /// the cell.
    ///
    /// The cell is used strictly as a consumer — only its update pipe is
    /// attached — so it follows the same contravariance as [#pipe(Pipe)]:
    /// a `Cell<Number>` can publish a flow's `Integer` outputs.
    ///
    /// @param cell the cell whose published value is updated by flow output emissions
    /// @return A pipe that applies this flow before publishing to `cell`
    /// @throws NullPointerException if cell is `null`
    /// @throws Fault                if the cell is not a runtime-provided implementation
    /// @see Cell#pipe()
    /// @see #pipe(Pipe)
    ...
    @SpecRef ( "6.2.6" )
    Pipe < I > pipe (
      @NotNull Cell < ? super O > cell
    );


    /// Returns a flow that emits a projection of each consecutive `(prev, curr)`
    /// relation over the output stream. The type-changing sibling of
    /// [Fiber#relate(Object, BinaryOperator)]: identical state semantics, with
    /// the relating function generalized from `(O, O) → O` to `(O, O) → P`.
    ///
    /// `op` may return `null` to filter the emission, so a gating use (emit only
    /// when consecutive values differ, are related in some way, or cross a
    /// boundary) is expressed by returning `null` on the cases that should not
    /// fire. Naming a transition between two values — "X became Y" → a single
    /// derived value — is one such use; the `(prev, curr)` pair is visible to
    /// `op` but never escapes, so the projected `P` is all that reaches
    /// downstream.
    ///
    /// ```
    /// emit(I) → ... → O → op(prev, curr) → P → downstream
    /// ```
    ///
    /// ## Lifecycle
    ///
    /// `prev` is seeded with `initial` at attach time (per [#pipe(Pipe)]
    /// materialization) and advances to the current value on **every**
    /// emission — including emissions for which `op` returns `null`. Filtering
    /// does not freeze `prev`; the next pair is always
    /// `(most-recent-value, next-value)`.
    ///
    /// ## Per Emission
    ///
    /// On each emission of `O`:
    /// 1. `p` captures the current `prev`
    /// 2. `prev` advances to the incoming value
    /// 3. `op.apply(p, curr)` produces the downstream value of type `P`
    /// 4. If `op` returns `null`, the emission is filtered (downstream sees nothing)
    ///
    /// ## Null Handling
    ///
    /// `initial` may be `null`; the first invocation then receives `null` as
    /// `prev`. `op` may return `null` to filter. Callers using a nullable seed
    /// must ensure `op` handles a `null` `prev` without throwing.
    ///
    /// ## Exception Handling
    ///
    /// If `op` throws, the current emission is dropped. `prev` has already
    /// advanced to the incoming value; the next emission proceeds from the
    /// advanced `prev`.
    ///
    /// ## Example: Naming a Status Transition
    ///
    /// ```java
    /// cortex.flow ( Status.class )
    ///   .relate (
    ///     Status.NOMINAL,
    ///     ( prev, curr ) ->
    ///       prev == curr            ? null
    ///       : curr == Status.SATURATED ? Transition.ENTERED_SATURATED
    ///       : prev == Status.SATURATED ? Transition.LEFT_SATURATED
    ///       : null
    ///   );
    /// ```
    ///
    /// @param initial the seed value for `prev` before the first emission
    /// @param op      the projection from `(prev, curr)` to output;
    ///                may return `null` to filter
    /// @param <P>     the output type
    /// @return A stateful flow projecting the consecutive-value relation
    /// @throws NullPointerException if `op` is `null`
    /// @see Fiber#relate(Object, BinaryOperator)
    ...
    /// Returns a flow that turns each admission into a [Run] envelope carrying the
    /// emission and its consecutive-run length. The length is `1` on the first
    /// admission and after every change, and increments while the value repeats.
    /// Consecutive repetition is decided by value equality ([Objects#equals]);
    /// for runs over a derived key, [#map(Function)] to the key first.
    ///
    /// The emitted [Run] is a fresh immutable envelope — safe to retain,
    /// compare, and forward.
    ///
    /// ```
    /// cortex.flow ( Sign.class )
    ...
    /// ```
    ///
    /// @return A stateful flow turning each `O` into a [Run] of `O`
    /// @see #change()
    /// @see Run
    /// @since 3.0
    ...
    /// Returns a flow that folds each emission into a running state and emits
    /// that state directly. Stateful per-materialization; state type `S` is
    /// independent of input `O`, and the downstream output is exactly `S`.
    ///
    /// This is the state-as-output form of [#scan(Supplier, BiFunction, Function)].
    /// It is equivalent to:
    ///
    /// ```java
    /// scan(initial, step, Function.identity())
    /// ```
    ///
    /// ## Publication Contract
    ///
    /// Because the scan state is emitted directly, `S` must be safe to publish
    /// downstream as a value. Use this overload for immutable or effectively
    /// immutable accumulator state: records, value objects, boxed values, and
    /// other states that downstream consumers cannot mutate after publication.
    ///
    /// A `step` function may return the same `S` reference when that reference
    /// is immutable and represents the correct next state. The API does not
    /// require or check reference freshness.
    ///
    /// For mutable working state, use [#scan(Supplier, BiFunction, Function)]
    /// and make the projection return an immutable snapshot or other safe
    /// public value. Publishing a mutable accumulator directly can leak the
    /// framework-owned state slot to downstream consumers.
    ///
    /// ## Cell Usage
    ///
    /// This overload is the concise way to publish an immutable running state to
    /// a [Cell]:
    ///
    /// ```java
    /// record Drawdown(double equity, double peak, double drawdown) {}
    ///
    /// Cell<Drawdown> cell = circuit.cell(new Drawdown(1.0, 1.0, 0.0));
    ///
    /// Pipe<Double> returns =
    ///   cortex.flow(Double.class)
    ///         .scan(
    ...
    ///           (s, r) -> {
    ///             double equity = s.equity() * (1.0 + r);
    ///             double peak = Math.max(s.peak(), equity);
    ///             return new Drawdown(equity, peak, (equity / peak) - 1.0);
    ///           })
    ///         .pipe(cell);
    /// ```
    ///
    /// Lifecycle, null handling, and exception handling are identical to
    /// [#scan(Supplier, BiFunction, Function)].
    ///
    /// @param initial the per-attachment factory for the initial state (invoked once at attach time)
    /// @param step    the binary operation `(state, input) -> next state`;
    ///                may return `null`
    /// @param <S>     the immutable/effectively immutable state and output type
    /// @return A stateful flow that emits its running state
    /// @throws NullPointerException if `initial` or `step` is `null`
    /// @see #scan(Supplier, BiFunction, Function)
    /// @see #flow(Function)
    /// @see Cell
    /// @since 2.8
    ...
    @NotNull
    @SpecRef ( "6.2.3" )
    default < S > Flow < I, S > scan (
      @NotNull final Supplier < ? extends S > initial,
      @NotNull final BiFunction < ? super S, ? super O, ? extends S > step
    ) {
    ...
    ///
    /// ```
    /// emit(I) -> this flow (I -> O) -> window(capacity) -> Window<O> -> downstream
    /// ```
    ///
    /// ## Window Semantics
    ///
    /// - A window is emitted after every surviving upstream `O` value.
    /// - Each emitted window contains at most `capacity` values.
    /// - Until `capacity` values have survived, the emitted window contains the
    ///   available prefix.
    ...
    /// values.
    ///
    /// @param capacity the maximum number of recent values retained per emitted window
    ...
    @New ( conditional = true )
    @NotNull
    @SpecRef ( {"6.2.3", "6.4", "6.4.1"} )
    Flow < I, Window < O > > window (
      int capacity
    );


    /// Returns a flow that emits a rolling [Window] of the output values
    /// observed within the last `duration`, limited to at most `capacity`
    /// retained values.
    ///
    /// `window(duration, capacity)` runs at the output side of this flow. Upstream
    /// stages first transform and filter the incoming `I` emission into a
    /// surviving `O`; old retained values are then evicted by processing time,
    /// the new value is appended, and a temporal window view is emitted
    /// downstream.
    ///
    /// ```
    /// emit(I) -> this flow (I -> O) -> window(duration, capacity) -> Window<O> -> downstream
    /// ```
    ///
    /// ## Window Semantics
    ///
    /// - A window is emitted after every surviving upstream `O` value.
    /// - Each emitted window contains values observed no earlier than
    ///   `duration` before the current emission's valve processing time.
    /// - Each emitted window contains at most `capacity` values.
    /// - Under high input rates, the capacity bound may evict older values that
    ///   are still within the duration bound.
    /// - Root windows emitted here are never empty: the current surviving
    ///   upstream value is always included.
    /// - The emitted root window's encounter order is oldest to newest.
    /// - The processing timestamp is captured from the circuit worker, not from
    ///   the external caller that submitted the value. It is the *stimulus time*
    ///   of the ingress chain: every emission a single ingress (external, ticker,
    ///   or cross-circuit) submission cascades into shares one reading, so internal
    ///   cause-effect within a cascade is co-temporal. Time advances per ingress,
    ///   not per internal transit hop.
    /// - Emitted windows are temporal views over a worker-thread-local ring; a
    ///   provider may reuse the same window object for later emissions.
    /// - A window must be consumed during the receiving callback. Receptors and
    ///   fibers must not store, emit, or pipe the window reference into another
    ...
    ///   more than once creates independent rolling windows.
    ///
    /// There is intentionally no duration-only overload. Time retention is not a
    /// memory bound; callers must choose an explicit capacity.
    ///
    /// @param duration the processing-time duration retained by each emitted window
    /// @param capacity the maximum number of recent values retained per emitted window
    /// @return A flow whose output is a time-bounded and capacity-bounded rolling [Window]
    /// @throws NullPointerException     if duration is `null`
    /// @throws IllegalArgumentException if duration is zero, negative, or cannot be represented in nanoseconds
    /// @throws IllegalArgumentException if capacity is less than 1
    /// @see Window
  ...
  ///
  /// @param <T> the type of values returned by this lookup
  /// @see Bank
  /// @see Pool
  /// @see Name
  /// @since 2.5
  ...
  @Abstract
  @SpecRef ( {"10.1", "16.2"} )
  sealed interface Lookup < T > {

    /// Returns the value for the given name.
    ///
    /// The creation, caching, and absence semantics depend on the implementation;
    /// the returned value is always non-null.
    ///
    /// @param name the name identifying the desired instance
    /// @return The value for this name (never null)
    /// @throws NullPointerException if name is null
    /// @throws Fault                if the name parameter is not a runtime-provided implementation
    /// @see Name
    /// @since 2.5
    ...
    /// Returns the value for the given substrate.
    ///
    ...
    /// @return The value for this substrate's name (never null)
    ...
    /// Returns the value for the given subject.
    ///
    ...
    /// @return The value for this subject's name (never null)
  ...
  ///
  /// **Depth limits**: A conformant implementation supports names of at least **16 segments**,
  /// whether that depth is reached in a single creation call or by successive extension. An
  /// implementation may support deeper names, and one that imposes a maximum must document it;
  /// exceeding that maximum raises an [IllegalArgumentException] from the creation or extension
  /// call rather than truncating the name or returning a shallower one. Portable code should not
  /// assume a depth beyond the guaranteed minimum; the provider in use documents its own maximum.
  ///
  ...
  interface Name
    ...
    /// @throws NullPointerException     if the path is `null`
    /// @throws IllegalArgumentException if the path is empty, contains empty segments, or the
    ///                                  extended name would exceed the implementation's maximum
    ///                                  depth (see [Name])
    ...
    ///                                  or the extended name would exceed the implementation's
    ///                                  maximum depth (see [Name])
  ...
  /// Indicates a Humainary-declared method whose invocation may allocate the
  /// instance it returns.
  ///
  /// Methods marked with this annotation are a source of allocation rather than a
  /// read of something already held; [#conditional()] distinguishes those that
  /// always allocate from those that may return an existing equivalent instead. In
  /// performance-critical code, the result should be hoisted outside hot loops to
  /// avoid repeated allocations.
  ///
  /// ## Conditional Allocation
  ///
  /// When [#conditional()] is `false` (the default), the method **always** allocates
  /// a new instance. When `true`, allocation depends on the arguments or on the
  /// receiver's state — the method may return an existing instance when the
  /// operation would produce an equivalent result.
  ///
  /// For example, [State#state(Name, int)] returns `this` when an equal slot already
  /// exists, and [Window#reverse()] returns `this` for an empty view. Such methods
  /// are marked `@New(conditional = true)`.
  ///
  /// ## Applying the Annotation
  ///
  /// The marking states what the **contract permits**, not what any one runtime does:
  ///
  /// - **Always allocates** — marked `@New`. A conforming runtime must return a
  ///   distinct instance; sharing one would break the contract.
  /// - **May reuse** — marked `@New(conditional = true)`. Callers must not rely on
  ///   the result being distinct, and a runtime is free to return an existing
  ///   instance. Immutable value derivations ([State], [Window], [Fiber], [Flow])
  ///   and interning factories ([Cortex#name(String)]) are marked this way as
  ///   families, whether or not a given runtime takes the shortcut.
  /// - **Never allocates** — left unmarked. Accessors ([Slot#name()],
  ///   [Subject#id()], [Circuit#current()]) and shared constants hand back
  ///   something the receiver already holds. A name-keyed retrieval is **not**
  ///   automatically one of these: [Lookup#get(Name)] leaves creation and caching
  ///   to the implementation, and [Bank#get(Name)] materializes on first access,
  ///   so both are conditional.
  ///
  /// An unconditional `@New` is therefore the stronger claim, and is wrong wherever
  /// a runtime is permitted to hand back an instance it already has.
  ///
  /// This classification covers methods declared by Humainary APIs. It does not
  /// classify methods merely inherited from JDK types, compiler-generated methods,
  /// or methods inherited from [Object] unless a Humainary API redeclares them.
  /// Inherited methods whose allocation behavior is material to the API are
  /// redeclared and marked explicitly, as with [Extent#spliterator()] and
  /// [State#iterator()].
  ...
  /// A circuit-owned, initialized state handle that grants immediate read/write
  /// access confined to the owning circuit context.
  ///
  /// Where [Cell] grants read + replace authority and [Port] grants queued
  /// transform authority without read, a Pin grants **immediate** inspect and
  /// mutate authority — but only inside the owning circuit context. The access
  /// guard rejects calls from any other context with [IllegalStateException].
  ///
  /// Use Pin when circuit-local code needs synchronous state access: when "set
  /// X, then read X on the next line" must work, or when mutable in-place
  /// objects must be updated under circuit-context confinement. Use [Cell]
  /// when readers from outside the circuit need observability. Use [Port] when
  /// any context should be granted queued mutation authority without read.
  ///
  /// ## Initialization
  ///
  /// A Pin is created via [Circuit#pin(Object)] or [Circuit#pin(Name, Object)]
  /// with a non-null seed value. Pins have no absent state — [#get()] is total.
  ///
  /// ## Owner-Context Guard
  ///
  /// [#get()] and [#set(Object)] are **immediate**, not queued. They MUST be
  /// invoked from the owning circuit's processing thread. Invocation from any
  /// other execution context — a caller context, a different circuit's context,
  /// or the cortex root context — MUST raise [IllegalStateException].
  ///
  /// The principled detection idiom mirrors the re-entrancy check used for
  /// [Circuit#await()] and [Circuit#pulse()]:
  ///
  /// ```java
  /// if ( cortex.current() != owningCircuit.current() ) {
  ///   // not on the owning circuit context — Pin access is illegal
  /// }
  /// ```
  ///
  /// ## Emission As Capability
  ///
  /// A Pin is itself a managed substrate handle and may be emitted as a value.
  /// Receivers outside the owning circuit may store, forward, or route the
  /// handle, but invoking [#get()] or [#set(Object)] from a non-owner context
  /// raises [IllegalStateException]. Dereferencing requires the handle to
  /// reach a pipe owned by the original circuit.
  ///
  /// ## Mutable Contents
  ///
  /// Within the owning circuit context, code may mutate a mutable object
  /// stored inside a Pin in-place. That mutation is safe only because the
  /// access is circuit-confined. A value obtained from [#get()] MUST NOT be
  /// retained, published, or mutated outside the owning circuit context
  /// unless that value independently satisfies the emission safety contract.
  ///
  /// Pin is not thread-safe through volatile or atomic access — safety comes
  /// from confinement, not synchronization. Cross-thread access from outside
  /// the owning circuit is prohibited by the guard, not coordinated.
  ///
  /// ## Performance
  ///
  /// The owner-context guard reduces to a single `Thread.currentThread()`
  /// intrinsic call plus a reference comparison against a worker thread
  /// reference cached at construction. JIT-compiled hot loops hoist the
  /// check across inlined call sites, so the per-access cost converges to
  /// the same shape as a plain field read. In JMH benchmarks, the measured
  /// delta between `pin.get()` / `pin.set()` and a baseline unguarded
  /// field access is below the noise floor (sub-15 picoseconds across
  /// multiple runs). Pin is safe to use inside per-emission callback loops
  /// without performance concern; the guard does not place Pin on a
  /// slower tier than untyped circuit-local state.
  ///
  /// @param <E> the type of state held by this pin
  /// @see Circuit#pin(Object)
  /// @see Circuit#pin(Name, Object)
  /// @see Cell
  /// @see Port
  /// @since 2.9
  ...
  @Provided
  @SpecRef ( {"11.6", "16.2"} )
  non-sealed interface Pin < E >
    extends Substrate < Pin < E > > {

    /// Returns the current stored value.
    ///
    /// Executes immediately on the owning circuit's worker thread; no queueing
    /// and no admission overhead. Always returns the present stored value —
    /// Pins are initialized and have no absent state.
    ///
    /// @return the current stored value; never `null`
    /// @throws IllegalStateException if called from any context other than the owning circuit's
    /// @since 2.9
    ...
    ///
    /// Executes immediately on the owning circuit's worker thread; the next
    /// statement in the same callback observes the new value.
    ///
  ...
  ///
  /// - [Circuit#pipe(Pipe)] - Admit emissions through a circuit before forwarding to a pipe
  /// - [Circuit#pipe(Receptor)] - Create a circuit-owned pipe that delivers emissions to a receptor
  /// - [Fiber#pipe(Pipe)] - Attach a same-type Fiber recipe before a pipe
  /// - [Fiber#pipe(Cell)] - Attach a same-type Fiber recipe before a cell update pipe
  /// - [Flow#pipe(Pipe)] - Attach a Flow before a pipe (with optional type change)
  /// - [Flow#pipe(Cell)] - Attach a Flow before a cell update pipe
  ///
  ...
  ///   the circuit thread
  /// - **Registered pipes**: Emitted to when their channel dispatches; delivery then
  ///   follows the pipe's own contract, as above
  ///
  ...
  /// - Providers must reject a null emission with [NullPointerException] before downstream delivery
  /// - Downstream receptors may rely on framework-delivered emissions being non-null
  ///
  ...
  /// - **[Sink]**: Returns pipes that funnel captures to a fixed endpoint; same derived-pool support
  /// - **Root pools** ([Cortex#pool(Function)]): Materialize values on demand from a factory function
  ///
  ...
  /// A circuit-owned, initialized state holder that grants queued mutation
  /// authority without exposing direct read access.
  ///
  /// Where [Cell] grants read + replace authority bundled together, a Port
  /// grants only queued mutation: replace, transform, transform-with-argument,
  /// and emission of the current value to a downstream pipe. The current value
  /// is never observable to the Port's holder; transformations execute inside
  /// the owning circuit context and the result remains circuit-confined.
  ///
  /// Use Port when a module needs to evolve circuit-owned state but should not
  /// receive the authority to read it. Use [Cell] when readers need direct
  /// observability of the latest published value. Use [Pin] when the state
  /// needs immediate inspect/mutate access from within the owning circuit
  /// context.
  ///
  /// ## Initialization
  ///
  /// A Port is created via [Circuit#port(Object)] or [Circuit#port(Name, Object)]
  /// with a non-null seed value. The seed represents the algebraic identity of
  /// the accumulation the Port carries — zero for a counter, an empty set for a
  /// unique-collection, and so on. Ports have no absent state.
  ///
  /// ## Queued Operations
  ///
  /// All Port operations are [Queued]. A call from any caller context returns
  /// before its state effect has been applied; the effect is processed on the
  /// owning circuit's worker thread. A call from the owning circuit context is
  /// transit work and is **also non-immediate** — code executing immediately
  /// after a Port operation in the same callback observes the *previous* value,
  /// not the new one. For circuit-local code that needs the mutation to be
  /// visible to subsequent statements in the same callback, use [Pin].
  ///
  /// This non-immediacy is not an implementation detail — it is required by
  /// the circuit's cascading-emission ordering (SPEC §5.3). A Port operation
  /// that executed inline from the owning worker would jump ahead of transit
  /// work already queued by earlier emissions, breaking the deterministic
  /// ordering guarantees that downstream observers depend on. The "set then
  /// read on the next line" semantic is therefore a Pin contract, not a
  /// missing Port feature.
  ///
  /// ## Failure Isolation
  ///
  /// If a transform function passed to [#update(UnaryOperator)] or
  /// [#update(Object, BiFunction)] returns `null` or throws, the Port retains
  /// its previous value and the failure is isolated under the circuit's
  /// existing external-callback failure path (SPEC §15.4) — sibling operations,
  /// subsequent queued work, and the circuit's dispatch loop are unaffected.
  /// The submitting caller does not observe the failure synchronously.
  ///
  /// When the failure is a `null`/absent return (a framework-detected
  /// contract violation rather than a user-thrown exception), the failure is
  /// surfaced as a [Fault] carrying the Port's [#subject()] and
  /// `operation = "update"`. This lets structured observers distinguish
  /// framework-detected violations from arbitrary exceptions thrown by user
  /// code without parsing exception messages.
  ///
  /// ## Mutable Values
  ///
  /// Port operations are replacement-oriented, not transactional rollback
  /// boundaries. Side effects or in-place mutations performed by a transform
  /// function before it throws are not undone. For this reason, Port values
  /// SHOULD be immutable or effectively immutable. Use [Pin] when code
  /// intentionally needs circuit-confined in-place mutation.
  ///
  /// @param <E> the type of state held by this port
  /// @see Circuit#port(Object)
  /// @see Circuit#port(Name, Object)
  /// @see Cell
  /// @since 2.9
  ...
  @Provided
  @SpecRef ( {"11.5", "16.2"} )
  non-sealed interface Port < E >
    extends Substrate < Port < E > > {

    /// Queues an emission of the current stored value to the supplied pipe.
    ///
    /// When processed on the owning circuit context, the Port reads its
    /// current value and invokes `pipe.emit(value)` against the supplied pipe.
    /// Because a Port is always initialized, emit always forwards one present
    /// value when the queued operation is processed.
    ///
    /// `Port.emit` does not copy, snapshot, or project the value; if
    /// transformation is needed before publication, compose a [Flow] or [Fiber]
    /// around the target pipe before passing it here.
    ///
    /// The supplied pipe must be a runtime-provided implementation; passing a
    /// pipe from a different provider raises [Fault] on the caller thread.
    ///
    /// @param pipe the target pipe (must be provider-compatible)
    /// @throws NullPointerException if pipe is `null`
    /// @throws Fault                if pipe is not a runtime-provided implementation
    /// @since 2.9
    ...
    /// Queues a whole-value replacement of the Port's stored value.
    ///
    /// When processed, the Port's stored value becomes `value`. The supplied
    /// value crosses a context boundary and SHOULD satisfy the emission safety
    /// contract: immutable, effectively immutable, or a managed substrate
    /// handle.
    ///
    /// @param value the replacement value
    /// @throws NullPointerException if value is `null`
    ...
    /// Queues a transformation of the Port's stored value.
    ///
    /// When processed on the owning circuit context, the Port invokes
    /// `fn.apply(current)` with the current stored value. If the function
    /// returns a non-null value, the Port stores that value. If the function
    /// returns `null` or throws, the Port retains its previous value and the
    /// failure is isolated as an external callback failure (see class docs).
    ///
    ...
    @SpecRef ( {"11.5", "15.4"} )
    void update (
      @NotNull UnaryOperator < E > fn
    );


    /// Queues a transformation that carries an explicit argument.
    ///
    /// When processed, the Port invokes `fn.apply(current, arg)`. The argument
    /// is captured at call time and supplied to the function on the worker
    /// thread, so callers that supply a non-capturing function (such as a
    /// static method reference) avoid allocating a capturing lambda.
    ///
    /// The argument crosses a context boundary and SHOULD satisfy the emission
    /// safety contract.
    ///
    /// Same failure-isolation semantics as [#update(UnaryOperator)].
    ///
    /// @param arg the argument supplied to the transformation
    /// @param fn  the transformation
    /// @param <A> the argument type
    /// @throws NullPointerException if arg or fn is `null`
    /// @since 2.9
    ...
    < A > void update (
      @NotNull A arg,
      @NotNull BiFunction < ? super E, ? super A, ? extends E > fn
    );
  ...
  ///
  /// All four readings are raw [System#nanoTime()] values — an arbitrary origin, and
  /// possibly negative — meaningful only for difference arithmetic, never as absolute
  /// instants.
  ///
  /// @see Circuit#pulse()
  /// @since 2.4
  ...
  interface Pulse {
    ...
    /// The reading taken when the probe began executing on the circuit context.
    ///
    /// @return circuit thread, when the probe began executing

    @SpecRef ( "5.7" )
    long dequeued ();


    /// The reading taken immediately after the probe submission returned.
    ///
    /// @return caller thread, immediately after the submit call returned

    @SpecRef ( "5.7" )
    long enqueued ();


    /// The reading taken immediately before the probe was submitted.
    ///
    /// @return caller thread, before the probe was submitted

    @SpecRef ( "5.7" )
    long start ();


    /// The reading taken after the probe completed, before this pulse was returned.
    ///
    /// @return caller thread, after the probe completed and the
  ...
  /// A temporary registration handle for attaching pipes to a named emission point during subscription.
  ///
  /// *Channel* is the role name for a named pipe acting as an inlet — there is no `Channel` type.
  ///
  ...
  /// named pipe identified by the subject.
  ///
  /// ## Temporal Validity
  ///
  ...
  @Temporal
  @Provided
  @SpecRef ( {"6.4", "6.4.1", "7.4", "16.2"} )
  interface Registrar < E > {

    /// Registers a pipe to receive emissions from the channel represented by this registrar.
    ///
    /// Each time the channel dispatches, the pipe is emitted to at its place in registration
    /// order, on the circuit's processing thread. Its delivery then follows the pipe's own
    /// contract: a pipe owned by this circuit is queued behind work the circuit has already
    /// accepted, and a pipe owned by another circuit is queued on that circuit. Dispatch never
    /// invokes the pipe's target directly; use [#register(Receptor)] for a direct call.
    ///
    /// ## Temporal Constraint
    ///
    /// **MUST be called only within the subscriber callback** that provided this registrar,
    /// on the circuit's processing thread that is running it. Calling after the callback
    /// returns, or from any other thread, including one the callback starts, is an illegal
    /// temporal use; implementations MUST detect this violation and signal an
    /// [IllegalStateException]. The performance escape
    ...
    /// ## Execution Guarantee
    ///
    /// The registered pipe's delivery runs:
    /// - On the processing thread of the circuit that owns the pipe (no synchronization needed)
    /// - In deterministic order with that circuit's other work
    /// - Sequentially, never concurrently with that circuit's other work
    ///
    /// ## Null Handling
    ///
    ...
    /// @throws IllegalStateException if called outside the valid subscriber callback, or from
    ///                               a thread other than the one running it
    /// @see Subscriber
    /// @see Pipe#emit(Object)

    @SpecRef ( {"6.3", "6.4", "6.4.1", "7.4"} )
    void register (
      @NotNull Pipe < ? super E > pipe
    );
    ...
    ///
    /// Each time the channel dispatches, the receptor is invoked directly at its place in
    /// registration order, on the circuit's processing thread, and completes before dispatch
    /// moves on to the next registration. It is not queued: it observes the emission ahead of
    /// work the circuit has already accepted, including the queued deliveries of same-circuit
    /// pipes registered on this channel. Use [#register(Pipe)] with a circuit pipe for a queued
    /// delivery instead.
    ///
    /// This overload has the same channel visibility, temporal validity, circuit-thread
    /// confinement and failure-isolation guarantees as [#register(Pipe)]. Implementations must
    /// not queue a receptor registration, since that changes the order in which it observes
    /// emissions relative to other accepted work.
    ///
    /// ## Temporal Constraint
    ///
    /// **MUST be called only within the subscriber callback** that provided this registrar,
    ...
    /// returns, or from any other thread, including one the callback starts, is an illegal
    /// temporal use; implementations MUST detect this violation and signal an
    /// [IllegalStateException]. See [#register(Pipe)] for
    /// the rationale.
    ///
    /// ## Common Use Cases
    ///
  ...
  /// subscribers, and subscriptions that hold system resources,
  /// maintain background operations, or need deterministic teardown.
  ///
  /// ## Core Implementations
  ///
  /// Required resource roles and types:
  /// - **[Circuit]**: Stops the processing thread and releases circuit-owned resources
  /// - **[Source]** ([Conduit]): closes source-owned subscriptions
  ...
  /// - State transitions: Active → Closing → Closed (terminal, see SPEC §9.3)
  ///
  ...
  ///   `Circuit.basin(int)`) raise [Fault] on the caller
  ...
  ///   resource can be identified at the catch site. Here *receiver* means the
  ///   resource whose own open-guard rejected the call, which is not always the
  ///   object the caller invoked: where a guarded operation is satisfied by
  ///   invoking another guarded operation on a different resource — [Bank#get(Name)]
  ///   materializing through its owning [Circuit], for instance — the fault is
  ///   raised and attributed by whichever resource observed itself as closed, and
  ///   providers do not re-attribute it to the outer receiver.
  ...
  non-sealed interface Resource < S extends Resource < S > >
    ...
    /// - **Circuit**: Stop processing thread and release circuit-owned resources
    ...
    /// [Subscription]), close operations are usually **queued** and
    ...
    ///
    /// For a resource managed by a Circuit, this operation must be invoked from
    /// outside that Circuit's processing context; otherwise waiting for the
    /// resource's close job would self-deadlock. The Java projection signals
    /// this illegal context use with [IllegalStateException].
    ///
    /// @throws IllegalStateException if invoked from the owning Circuit's processing context
    /// @since 2.5
  ...
  /// An immutable snapshot of a consecutive run of value-equal emissions,
  /// produced per admission by [Flow#run()]. It carries the repeated [#emission()]
  /// and the running [#length()] — the number of consecutive value-equal
  /// admissions ending at this one (`1` on the first admission and after every
  /// change, incrementing while the value repeats).
  ///
  /// Each admission produces a fresh, immutable envelope — a pure
  /// out-of-execution-scope record, safe to retain, compare, and forward with no
  /// handle to any live counterpart (the [Capture] carrier pattern).
  ///
  /// @param <E> the emission type
  ...
  interface Run < E > {
    ...
    /// Returns the emission whose consecutive run this envelope describes.
    ///
    /// @return the repeated emission
    ...
    /// Returns the running length: the number of consecutive value-equal
    /// admissions ending at this one — `1` on the first admission and after every
    /// change, incrementing while the value repeats.
    ///
    /// @return the consecutive-run length, always `>= 1`
    ///
    /// Lengths use Java's signed 64-bit `long`. A run exceeding [Long#MAX_VALUE]
    /// is outside this projection's representable range; callers must not rely on
    /// wraparound as a valid run length.
  ...
  non-sealed interface Scope
    ...
    ///
    /// The Java projection performs cleanup in this order:
    ...
    /// - Closes before the parent’s `close()` returns; see [#close()] for cleanup order
  ...
  /// A circuit-owned pool of named pipes that funnels every emission, enriched as a
  /// [Capture], into a single endpoint pipe fixed at creation.
  ///
  /// Sink is the **output-closed dual of [Conduit]**. Both are a [Pool] of [Pipe]:
  ///
  /// ```
  /// Conduit<E> = Pool<Pipe<E>> + Source<E, Conduit<E>>   // output open:   consumers subscribe
  /// Sink<E>    = Pool<Pipe<E>> + Substrate<Sink<E>>       // output closed: producer fixes the endpoint
  /// ```
  ///
  /// A [Conduit] leaves its output open — emissions are dispatched to whatever subscribers
  /// attach at runtime, and each subscriber receives the raw value. A Sink inverts this: the
  /// destination is a single [Pipe] of [Capture] supplied at construction and there is no
  /// [Source] to subscribe to. Each value emitted into one of the sink's named pipes is wrapped
  /// in a [Capture] — bearing the emitting pipe's [Capture#subject()], the [Capture#current()]
  /// emitting context, the [Capture#emission()] value, and any provider [Capture#state()] — and
  /// forwarded to the endpoint.
  ///
  /// ## Minting Captures
  ///
  /// A Sink is a **capture-producing pool of channels**: each [Capture] is minted at the sink
  /// channel the value passes through, which owns and stamps it — just as a [Conduit] pipe owns its
  /// emissions. Because only the runtime can observe the per-emission emitting context, the minted
  /// capture carries
  /// [Capture#current()] for free — provenance a hand-written subscriber cannot reconstruct, since it
  /// can recover a channel's [Capture#subject()] but never the emitting context of each emission.
  ///
  /// ## Endpoint
  ///
  /// The endpoint is any [Pipe] of [Capture] and need not belong to this sink's circuit: a
  /// capture is an out-of-execution-scope record, so forwarding it to another circuit is an
  /// ordinary cross-circuit emission, and when the endpoint already belongs to this circuit the
  /// usual same-circuit pipe optimization applies. The endpoint is **not owned** by the sink:
  /// the sink never closes it.
  ///
  /// ## Pool Semantics (via Pool)
  ///
  /// Identical to [Conduit]: [Pool#get(Name)] returns a cached [Pipe] of `E` per name and
  /// [Pool#pool(Function)] derives cached views — including pre-capture transformation and
  /// filtering via `pool(flow::pipe)` or `pool(fiber::pipe)`. A value suppressed by such a flow
  /// or fiber never reaches the sink's pipe and so mints no capture; the capture's
  /// [Capture#subject()] is always the sink's [Pipe] of `E`, taken after any such processing.
  ///
  /// A sink can also be passed directly to [Circuit#subscriber(Name, Pool)] so a source's
  /// discovered channels are wired into same-named sink channels:
  ///
  /// ```java
  /// Sink<Integer> sink = circuit.sink(endpoint);
  /// source.subscribe(circuit.subscriber(name, sink));
  ...
  /// ## Lifecycle
  ///
  /// A Sink is a [Substrate], not a [Resource]: it owns nothing that needs releasing — its
  /// channels are ordinary circuit pipes (valid while the owning circuit is open, reclaimed by
  /// the garbage collector once unreachable) and the endpoint is caller-owned. There is no
  /// `close()`; a channel handle keeps forwarding captures until the owning circuit closes.
  ///
  /// ## Flow Control
  ///
  /// A Sink has no disable or `close()` of its own, and needs none — gating composes around it with
  /// ordinary [Fiber]/[Pipe] operators, and the sink is oblivious to either. Filter the **input** with
  /// [Pool#pool(Function)] — e.g. `pool(cortex.fiber(E).guard(p)::pipe)` — so a suppressed value never
  /// reaches a channel and mints no capture. Or filter the **output** by wrapping the endpoint before
  /// construction — `circuit.sink(fiber.pipe(endpoint))` — so captures are rate-limited or dropped
  /// before the real endpoint, transparently to the sink. To stop flow entirely: cease emitting, close
  /// the bridging [Subscription] when the sink is fed from a source, or close the owning circuit.
  ///
  /// @param <E> the class type of emitted values wrapped into captures
  /// @see Capture
  /// @see Conduit
  /// @see Basin
  /// @see Circuit#sink(Pipe)
  /// @see Pool
  /// @see Pipe
  /// @since 2.10
  ...
  ///
  /// This Java projection compares slot types by [Class] reference identity. That
  /// identity is guaranteed only within one Java class-loader/module domain; slot
  /// matching across independently loaded copies of an application type is not
  /// supported. Applications crossing that boundary must adapt values to a shared
  /// type representation before constructing or querying slots.
  ///
  ...
  ///
  /// The required source type is [Conduit]. The
  ...
  interface Source < E, S extends Source < E, S > >
    ...
    /// Subscribes a [Subscriber] to receive lazy callbacks during named-pipe rebuild.
    ///
    /// *Channel* throughout this method's documentation names the inlet role of a named pipe,
    /// not a separate type — there is no `Channel` type in the API.
    ///
    /// The subscriber callback is invoked on the circuit's processing thread during rebuild,
    /// which occurs when a named pipe receives its first emission after the subscription is
    /// registered. This allows the subscriber to dynamically register pipes to receive
    /// emissions from that named pipe.
    ///
    /// ## Subscription Lifecycle
    ///
    /// The returned [Subscription] remains active until:
    /// - [Subscription#close()] is called explicitly
    /// - The source itself is closed — via [Conduit#close()]
    ///
    /// ## Registration Timing
    ///
    ...
    ///   within the window
    /// - **Visibility window**: A subscription receives exactly those emissions a channel
    ///   processes between the execution of its registration job and the execution of its close
    ///   job, in the circuit's logical processing order (SPEC §7.6.1). With no cascade in play
    ///   this is the interval between the two enqueue positions.
    ...
    /// - The source itself closed (e.g., via [Conduit#close()])
    ///
    /// ## Callback Guarantees
    ///
    ...
    ///   circuit's forward progress, are unaffected. Failure reporting follows the provider
    ///   mechanism and the conditional structured-reporting requirements described in [Fiber].
    ///
    /// ## Typical Uses
    ///
  ...
  interface State
    ...
    /// Returns an iterator over this state's slots from the most recently written
    /// entry to the oldest.
    ///
    /// A non-empty state requires an independent traversal cursor. An empty state
    /// may return a shared empty iterator.
    ///
    /// @return An iterator over this state's slots in iteration order
    ...
    /// Returns a spliterator over this state's slots from the most recently written
    /// entry to the oldest.
    ///
    /// A non-empty state requires an independent traversal cursor. An empty state
    /// may return a shared empty spliterator.
    ///
    /// The Java projection requires the encounter order above but does not require
    /// any [Spliterator] characteristic flags. Implementations may report
    /// characteristics such as [Spliterator#SIZED], [Spliterator#SUBSIZED], or
    /// [Spliterator#IMMUTABLE] when available; callers must not depend on them.
    ///
    /// @return A spliterator over this state's slots in iteration order
  ...
  /// A lifecycle handle — not a callback — created via [Circuit#subscriber(Name, BiConsumer)];
  /// behavior is supplied as `BiConsumer<Subject<Pipe<E>>, Registrar<E>>` at construction time.
  ///
  /// The handle itself owns lifecycle and subscription state, while the callback decides which
  /// [Pipe]s receive emissions from channels discovered in a [Source].
  ///
  ...
  /// *same* circuit — that is, any [Conduit] whose owning circuit matches the
  /// subscriber's. Subscribing to a
  ...
  /// callback is not retried for that subscription/channel pair. Registration calls completed
  /// before the failure remain registered (see SPEC §15.4 invariant 3 and
  /// [Circuit#subscriber(Name, BiConsumer)]).
  ///
  ...
  /// - **Testing**: Capture retained emissions for validation
  ///
  /// ## Resource Lifecycle
  ///
  ...
  /// owning circuit, and returns immediately. The close does not take effect at
  /// the moment of return. Work already ahead of the close job may still be
  /// delivered first; the close effect becomes observable only after the circuit
  /// context has processed the close job. No cleanup runs on the caller thread;
  /// if the owning circuit is already fully closed and cannot accept the close
  /// job, unregistration and `onClose` notification for that late request may
  /// never be processed. A caller that needs to confirm an asynchronous close
  ...
  /// lifecycle work.
  ///
  /// **Which queue the close job joins depends on where `close()` was called
  /// from**, not on the fact that it is a close (SPEC §5.3). Called from within
  /// a subscriber callback or a registered receptor of the owning circuit, it is
  /// **transit** work and takes effect inside the current cascade. Called from
  /// any other context — an external thread, or another circuit's context — it
  /// is **ingress** work and takes effect after the current cascade has drained.
  ///
  /// Closing never retracts a delivery that has already been selected. A
  /// subscriber that closes its own subscription from inside one of its
  /// receptors still lets the deliveries the current emission already selected
  /// run, including those to its own sibling registrations.
  ///
  /// ## Visibility Window
  ///
  /// A subscription's **visibility window** runs from the point its registration
  /// job **executes** to the point its close job executes, in the circuit's
  /// logical processing order — transit ahead of ingress, each FIFO within
  /// itself. When a channel processes an emission it selects the registrations
  /// of exactly those subscriptions effective at that point:
  ///
  /// - Emissions a channel processes **before** the subscribe job are not
  ///   visible — the registration has not been installed when they run.
  /// - Emissions processed **within the window** are delivered in order.
  /// - Emissions processed **at or after** the close job are not visible — the
  ///   close takes effect at its own position, and subsequent emissions trigger
  ///   a lazy rebuild that excludes the closed subscription.
  ///
  /// The window is deliberately **not** defined over enqueue positions, because
  /// work raised inside a cascade has none. A callback that emits, subscribes,
  /// and emits again leaves transit holding `[E1, register(S), E2]`; processing
  /// `E1` appends `E1a` behind `E2`, so `S` misses `E1` and receives both `E2`
  /// and `E1a` — even though `E1a` descends from the emission `S` never saw.
  /// Causal ancestry does not decide visibility.
  ///
  /// For a single caller thread with no cascade in play, the rule reduces to
  /// enqueue order, which is how it is usually read:
  /// `subscribe(); emit(X); close(); emit(Y); await()` delivers `X` but not `Y`.
  /// Across concurrent caller threads the relative enqueue positions are
  /// determined by the ingress mechanism and may differ across runs, but each
  /// subscription's window is still defined by its own registration and close
  /// positions.
  ///
  ...
  /// that preserves the visibility rule — the version increment is itself an
  /// operation in the processing order, taking effect at its own position.
  ///
  /// ## Resubscription
  ///
  ...
  /// A handle controlling the lifecycle of a scheduled emitter created by
  /// [Circuit#ticker(Duration, Pipe)].
  ///
  /// A Ticker drives periodic emissions of monotonically increasing sequence
  /// numbers into a target pipe. Each tick is submitted through its owning
  /// circuit's queue before being forwarded to the target pipe, preserving
  /// ordering with the owner's queued work.
  ///
  /// ## Emission Origin
  ///
  /// A ticker is a circuit-internal mechanism: its scheduling thread is an
  /// implementation detail and is never surfaced to the application. Tick
  /// emissions are therefore attributed to the **owning circuit**, not to the
  /// scheduler — a tick into a same-circuit target reports [Capture#current()]
  /// equal to the owner's [Circuit#current()], exactly as a cascade does; a tick
  /// into a cross-circuit target reports the owner circuit's context as its
  /// emitting context. Consumers that need to recognize tick-driven emissions use
  /// [Capture#subject()] (the target pipe), not the context.
  ///
  /// ## Resource Lifecycle
  ///
  /// As a [Resource], the Ticker is closed to stop scheduled emissions.
  /// Closing the owning circuit rejects future tick submissions; callers that
  /// want deterministic ticker shutdown should close the ticker directly or
  /// register it with a [Scope]. Target receptor failures are isolated by the
  /// executing circuit and do not close the ticker.
  ///
  /// @see Circuit#ticker(Duration, Pipe)
  /// @see Resource
  /// @since 2.8
  ...
  /// window emitted by `Flow.window(capacity)` is never empty: the first surviving
  ...
  /// only for that callback, and only on the thread running it: providers may back
  /// it with a mutable ring reused for later emissions, but each callback receives
  /// its own view. Receptors and fibers must not store, emit, or pipe a Window
  /// reference into another pipeline, or hand it to another thread. Callers that need to retain or forward values must copy
  ...
  /// Each Window operator verifies that the callback that received this view is
  /// still running, and that it is called on the thread running that callback. A
  /// view observed after the originating callback returned — for example a view
  /// stored in a field, forwarded to another pipe, or passed to an asynchronous
  /// consumer — throws [Fault] on the next operator invocation, and so does a view
  /// used from another thread while the callback is still running. The fault's [Fault#subject()]
  ...
  /// may use object identity; do not use Window references as map keys or set
  /// members to represent observed contents.
  ///
  ...
  ///
  /// Provider selection occurs once during class initialization. Changing the provider property
  /// cannot repair failed initialization in the same class loader. Errors thrown directly by
  /// discovery or provider code may propagate without an initialization wrapper.
  ///
  /// @return The singleton Cortex instance
  /// @throws ExceptionInInitializerError if provider initialization fails with an exception;
  ///                                     inspect its cause for the selection or construction failure
  /// @throws NoClassDefFoundError        if provider initialization previously failed in this class loader
  /// @see Cortex
  ...
  /// Structured fault for substrate runtime errors detected by a provider.
  ///
  ...
  ///
  /// *Receiver* is per-operation, not per-call-site: [#subject()] and [#operation()]
  /// name the substrate that rejected the call and the operation it refused, which
  /// need not be the method the caller invoked. Where one guarded operation is
  /// satisfied by invoking another on a different substrate — [Bank#get(Name)]
  /// materializing through its owning [Circuit], for instance — the inner rejection
  /// propagates out unchanged rather than being re-attributed to the outer receiver,
  /// so a catch site reads the pair together to see what actually refused the work.
  ///
  /// ## Common Causes
  ///
  ...
  final class Fault
    ...
    /// The receiver/context where the call failed — the substrate whose own guard
    /// rejected the call, which is not necessarily the object the caller invoked.
    /// See the class documentation for how a rejection reached through a delegating
