...
6. **Filter or transform** — attach a `Fiber` recipe (`cortex.fiber(T.class).guard(...).diff()`)
   for per-emission operators, or a `Flow` (`cortex.flow(I.class).map(...).fiber(...)`) for
...
- **Flow**: Type-changing transformation pipeline composing Fibers with mappers (map, fiber,
  flow, pipe)
...
- Per-attachment Fiber operators maintain independent state per materialization, ensuring
  stateful operators (diff, reduce, limit) are isolated per named pipe
