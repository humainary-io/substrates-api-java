...
final class CellTest
  ...
  ///   ↓
  /// Outlet: circuit.pipe(emissions::add) → captures 42
  ///
  /// Result: emissions = [42]
  /// ```
  ///
  ...
  /// ```java
  /// cell.receive(value);  // Cell acts as emission receiver
  /// ```
  ///
  /// Cell as Source:
  ...
  /// same.receive(value);             // Subscriber receives emission (same cell)
  /// ```
  ///
  /// Hierarchical Emission Flow:
  /// ```
  /// grandchild.receive(value)
  ///   ↓
  ...
  ///
  /// child1.receive(10);  // Subscriber receives
  /// child2.receive(20);  // Subscriber receives
  /// child3.receive(30);  // Subscriber receives
  ///
    ...
    try {
      ...
      rootCell.subscribe (
        circuit.subscriber (
          ...
          ( _, registrar ) ->
            registrar.register ( subscriptionEmissions::add )
        )
      );
    ...
    try {
      ...
      rootCell.subscribe (
        circuit.subscriber (
          ...
          ( _, registrar ) ->
            registrar.register ( allEmissions::add )
        )
      );
  ...
  void testCellWithCircuitPipe () {
    ...
    try {
      ...
      assertEquals (
        List.of ( 123 ),
        emissions,
        "Cell should work with receptor for async dispatch"
      );
    ...
    try {
      ...
      final Cell < Integer, String > cell =
        circuit.cell (
          ...
          channel -> circuit.pipe ( ( Integer n ) -> channel.pipe ().emit ( String.valueOf ( n * 2 ) ) ),
  ...
  ///
  /// grandchild.receive(event) → converts to Double
  ///   ↓
    ...
    try {
      ...
      final var rootCell =
        circuit.cell (
          ...
          identity (),
          identity (),
          Receptor.of ( Integer.class, emissions::add )
        );
  ...
  void testMultipleChildrenEmitToParent () {
    ...
    try {
      ...
      final Cell < Integer, Integer > parentCell =
        circuit.cell (
          cortex.name ( "parent" ),
          identity (),
          identity (),
    ...
    try {
      ...
      final Cell < Integer, String > cell =
        circuit.cell (
          ...
          channel -> circuit.pipe ( ( Integer n ) -> channel.pipe ().emit ( "Value: " + ( n + 100 ) ) ),
          identity (),  // egress: identity
          emissions::add
        );
